...
write_csv(here("processed-data", "allcurves_clean_v2.csv"))
```
...
swc_field_data_all <- field_data_all %>% 
  mutate(gww_gdw = )
  filter(water_potential > tlp) %>% #for points above the tlp (estimated to be around ~2)
  group_by(individual) %>% 
  summarise(swc_sat = lm(formula = gww_gdw ~ (-1 * water))$coefficients[["(Intercept)"]]) 
