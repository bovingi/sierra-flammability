...
pv.summary.spp <- pv_allcurves_data_clean %>%
  ...
  filter(spp != "ADFA") %>% 
  filter(spp != "CEME") %>%
  mutate(spp = recode(spp, ABCO = "Abies concolor", ARPA = "Arctostaphylos patula", CADE = "Calocedrus decurrens", CECO = "Ceanothus cordulatus", PIJE = "Pinus jeffreyii", QUKE = "Quercus kelloggii")) %>% 
  group_by(spp) %>% 
  summarise(n=n(), 
            mean_tlp=mean(tlp),
            sd_tlp=sd(tlp), 
