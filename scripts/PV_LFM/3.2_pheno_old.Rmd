...
# Morgenstern = list(c("#7c668c", "#b08ba5", "#dfbbc8", "#ffc680", "#ffb178", "#db8872", "#a56457"), c("darkpurp", "medpurp", "lightpurp", "yellow", "#E69512en", "orange", "darkorange"), colorblind=TRUE)
# 
...
npn_df <- read_csv(here("processed-data", "analysis 2.0", "npn_data.csv")) # National phenology network dataset
```
...
p2 <- ggplot2::ggplot(data = df3, aes(x = date, y = lfm, color = phenophase_name)) +
  ...
  scale_color_manual(values = c("#4ECDC4", "#2B3E37", "#4B4975")) +
...
legend.plot <- ggplot() +
  ...
  labs(color = "Phenophase") +
  scale_color_manual(values = c("#4ECDC4", "#3A5C3E", "#2B3E37", "#E69512", "#6B91BD", "#4B4975", "#D3225D")) + #Holder color palette, matched to the phenophase vector above
...
arpa.colors <- c("#4ECDC4", "#D3225D", "#4B4975", "#3A5C3E")
...
ceco.colors <- c("#D3225D", "#4B4975", "#3A5C3E")
...
p.spp.main
ggsave(here("figures", "phenophase.lfm.boxplot.main.jpg"),
       height = 10, width = 15,
       plot = p.spp.main)
# For some strange reason, I could not get the legends and the plots to mesh together with cowplot or ggarrange... I ended up just stitching them together on Google Slides
```
...
field_data_2021%>% 
 ...
 # geom_point(color = "olivedrab", alpha = .5, size = .5) +
  ...
  geom_jitter(aes(x= date, y = predawn), color = "#4ECDC4", alpha = .5, size = .5) 
```
