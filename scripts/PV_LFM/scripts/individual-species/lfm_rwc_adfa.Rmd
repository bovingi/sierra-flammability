...
Look at just one species and timepoint: ADFA and fall
...
ggplot(pv.ADFA.fall.summary) +
   ...
   labs(title = "ADFA TLP, fall dates") +
   theme_bw()
```
...
pv.ADFA.fall %>% 
  ...
  geom_point() +
  geom_hline(yintercept = ADFA.tlp,
             colour = "cyan") +
  labs(title = "ADFA Pressure Volume Curves, LFM x Mpa")
  theme_bw()
```
...
pv.ADFA.fall.summary <- data %>%
 ...
 ggplot(pv.ADFA.fall.summary) +
   ...
   labs(title = "ADFA TLP with sd") +
   theme_bw()
```
Subset data based on TLP: 

```{r}
ADFA.above.tlp <- pv.ADFA.fall %>% 
  filter(water_potential > tlp) 
  #filter(lfm < 200, lfm > 90)

ADFA.below.tlp <- pv.ADFA.fall %>% 
  filter(water_potential < tlp)
  #filter(lfm < 200, lfm > 90)
```
...
fit.below <-lm(-1/water_potential~lfm, ADFA.below.tlp)
summary(fit.below)
...
```{r}
ggplot(data = ADFA.above.tlp, aes(y = water_potential, x = lfm, color = sample, shape = as.factor(date))) +
  geom_jitter(alpha = .6, size = 1) +
  geom_smooth(method = "lm", se = FALSE, size = .5, alpha = .5) +
  theme_bw()

fit.above <- lm(water_potential ~ lfm, ADFA.above.tlp)
summary(fit.above)
...
```{r}
fit2 <- lm(water_potential ~ lfm + swc, ADFA.above.tlp)
summary(fit2)
...
We're just interested in ADFA, so just look there: 

```{r}
flam_curve_phys_ADFA_above <- flam_curve_phys_all %>% 
 # filter(rwc_new < 100) %>% 
  filter(spp == "ADFA") %>% 
  filter(mpa > tlp)

flam_curve_phys_ADFA_below <- flam_curve_phys_all %>% 
 # filter(rwc_new < 100) %>% 
  filter(spp == "ADFA") %>% 
  filter(mpa < tlp)
```
...
```{r}
predict.below <- predict(fit.below, newdata = flam_curve_phys_ADFA_below, se.fit = TRUE, interval = "confidence") #make prediction

# Bind to the data to make it actually useful:

predict.df.below <- data.frame(flam_curve_phys_ADFA_below, predict.below)
...
predict.df.below %>% 
  ...
  ylim(0, 1) +
  xlim(0, 1) +
  labs(title = "ADFA.Actual vs. predicted water potential for points BELOW the TLP", 
       y = "Actual -1/Mpa", 
       x = "Predicted -1/Mpa")
...
predict_graph_below <- ggplot(predict.df.below, aes(x = lfm, y = -1/fit.fit)) +
  ...
  geom_point(data = flam_curve_phys_ADFA_below, aes(x = lfm, y = mpa), alpha = 0.5) +
 # facet_wrap(~St) +
  labs(x = "LFM (%)", 
       y = "Water Potential (Mpa)", 
       title = "Predicted (cyan) vs. observed (black) LFM and Mpa (ADFA)") +
...
```{r}
predict.above <- predict(fit.above, newdata = flam_curve_phys_ADFA_above, se.fit = TRUE, interval = "confidence") #make prediction

# Bind to the data to make it actually useful:

predict.df.above <- data.frame(flam_curve_phys_ADFA_above, predict.above)
...
predict_graph_above <- ggplot(predict.df.above, aes(x = lfm, y = fit.fit)) +
  geom_point(color = "cyan", alpha = 0.5) +
  geom_point(data = flam_curve_phys_ADFA_above, aes(x = lfm, y = mpa), alpha = 0.5) +
 # facet_wrap(~St) +
  labs(x = "LFM (%)", 
       y = "Water Potential (Mpa)", 
       title = "Predicted (cyan) vs. observed (black) LFM and Mpa (ADFA)")
