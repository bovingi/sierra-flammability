...
tlp_summary_month <-pv_allcurves_data_clean %>%
  mutate(month_char = as_character(month)) %>% 
...
```{r, message = FALSE}
field_data_2021_raw <- read.csv(here("raw-data", "Summer2021.LFM.mpa.data.analysis.csv")) %>%
  clean_names() 
...
field_data_2021 <- field_data_2021_raw %>% 
  ...
  mutate(rwd_wettest = 100 - rwc) %>% 
  mutate(water_potential = midday) %>% 
  mutate(swc = wettest_lfm/100)

field_data_2021_tlps <- merge(merge(tlp_summary_spp, tlp_summary_spp_lfm, by = "spp", all = T), field_data_2021, by = "spp", all = T)

field_data_2021_pv <- merge(merge(tlp_summary_timing, tlp_summary_timing_lfm, by = c("spp", "timing"), all = T), field_data_2021_tlps, by = c("spp", "timing"), all =T) #add in mean TLP for each species

field_data_2021_pv <- merge(tlp_summary_spp_lfm, field_data_2021_tlps, by = "spp", all = T) #add in mean LFM at TLP for each spp

#field_data_2021_pv %>% 
#write_csv(here("processed-data", "field_data_2021_csv"))
...
dlookr::diagnose(field_data_2020)
```


```{r, message = FALSE}
field_data_2020_tlps <- merge(merge(tlp_summary_spp, tlp_summary_spp_lfm, by = "spp", all = T), field_data_2020, by = "spp", all = T)

field_data_2020_pv <- merge(merge(tlp_summary_timing, tlp_summary_timing_lfm, by = c("spp", "timing"), all = T), field_data_2020_tlps, by = c("spp", "timing"), all =T) #add in mean TLP for each species

field_data_2020_pv <- merge(tlp_summary_spp_lfm, field_data_2020_tlps, by = "spp", all = T)%>% 
...
We probbably also want to get swc to be the extrapolated value, so....


This is effectively what we're trying to do to get the saturated water content except we'll do it for each individual, not for each species. 

```{r}
ggplot(field_data_all, aes(x = -1*mpa, 
                              y = gww_gdw, 
                              color = spp)) +
  geom_point(alpha = .3) +
  geom_smooth(method = "lm", se = FALSE, size = .5) +
...
swc_field_data_all <- field_data_all %>% 
  ...
  group_by(spp) %>% 
  summarise(swc_sat = lm(formula = gww_gdw ~ (-1 * mpa))$coefficients[["(Intercept)"]]) 
