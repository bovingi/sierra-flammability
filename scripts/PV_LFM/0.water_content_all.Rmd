...
pv_segs_kable <- pv_segs %>% 
  mutate(species = case_when(
    species == "ABCO" ~ "Ab. concolor",
    species == "CADE" ~ "Ca. decurrens",
    species == "PIJE" ~ "Pi. jeffreyii",
    species == "CECO" ~ "Ce. cordulatus",
    species == "ARPA" ~ "Ar. patula",
    species == "QUKE" ~ "Qu. kelloggii",
  )) %>% 
  mutate_if(is.numeric, round, digits = 2)
pv_segs_kable$species <- factor(pv_segs_kable$species, levels = c("Ab. concolor", "Pi. jeffreyii", "Ca. decurrens", "Ar. patula", "Ce. cordulatus", "Qu. kelloggii"))
```

Kable
```{r}
pv_segs_kable %>% kable(format = 'html', escape = F, col.names = c('Species', 'Year', 'LFM @ TLP', 'Standard Dev.', 'TLP', 'Standard Dev.', 'RWC @ TLP', 'Standard Dev.', paste('P', "\U2080"), 'Standard Dev.', 'e', 'Standard Dev.')) %>% 
  ...
  save_kable(here('figures', 'pv.curve.summary.html'))
```
...
#ggsave(table_pretty, here("figures", "PV_summaries_year"), device = "jpeg")
```
