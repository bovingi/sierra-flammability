...
mean_tlp <- field_data_all_4 %>% 
  drop_na(mean_tlp_spp) %>% 
 # filter(year == 2021, timing == "fall") %>% 
  mutate(mean_tlp_all = mean(mean_tlp_spp)) %>% 
  select(mean_tlp_all) %>% 
  distinct() %>% 
  pull()
mean_tlp
...
sd_tlp <- field_data_all_4 %>% 
  drop_na(mean_tlp_spp) %>% 
 # filter(year == 2021, timing == "fall") %>% 
  mutate(sd_tlp_all = sd(mean_tlp_spp)) %>% 
  select(sd_tlp_all) %>% 
  distinct() %>% 
  pull()
sd_tlp
...
ggplot(aes(x = mpa, 
  ...
  #geom_vline(xintercept  = field_data_angio$mean_tlp_spp, colour = "blue") +
 # geom_vline(xintercept  = field_data_gymno$mean_tlp_spp, colour = "purple") +
  geom_vline(xintercept  = mean_tlp, colour = "green", size = 2) +
...
#With random
field_segment_df <- field_segment_angio_df %>% 
  drop_na(mpa, lfm) %>% 
  mutate(id = unique_id)

o_mod<-lme(lfm ~ mpa + spp + doy, random=~1|id, data=field_segment_angio_df)
...
#No random
u_mod <- lm(lfm~mpa+ spp + doy, data = field_segment_angio_df)
u_mod_b <- segmented(u_mod, ~mpa)
summary.segmented(u_mod_b)
...
mean_tlp <- field_segment_angio_df %>% 
  drop_na(mean_tlp_spp) %>% 
 # filter(year == 2021, timing == "fall") %>% 
  mutate(mean_tlp_all = mean(mean_tlp_spp)) %>% 
  select(mean_tlp_all) %>% 
...
ggplot(aes(x = mpa, 
 ...
 # geom_vline(xintercept  = field_data_gymno$mean_tlp_spp, colour = "purple") +
  ...
  geom_ribbon(aes(xmin = lwr_tlp, 
              ...
              alpha = .5)  +
  labs(title = "Angiosperms", 
       caption = "pink = est. breakpoint with no random effects, red = with random effects, green = tlp")
```
...
#With random
field_segment_df <- field_segment_gymno_df %>% 
  drop_na(mpa, lfm) %>% 
  mutate(id = unique_id)
...
ggplot(aes(x = mpa, 
  ...
  geom_vline(xintercept  = seg_psi_df$mpa_psi, color = "pink", size = 2) +
   geom_vline(xintercept  = seg_psi_norand_df$mpa_psi, color = "red", size = 2) +
  geom_vline(xintercept  = seg_psi_norand_df$mpa_psi, linetype = "dotted") +
  #geom_vline(xintercept  = field_data_gymno$mean_tlp_spp, colour = "blue") +
 # geom_vline(xintercept  = field_data_gymno$mean_tlp_spp, colour = "purple") +
  geom_vline(xintercept  = mean_tlp, colour = "green", size = 2) +
  geom_ribbon(aes(xmin = lwr_tlp, 
                  xmax = upr_tlp), 
