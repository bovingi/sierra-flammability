...
#With random
field_segment_df <- field_data_all %>% 
  drop_na(mpa, lfm) %>% 
  mutate(id = unique_id)
...
ggplot(aes(y = mpa, 
  ...
  geom_vline(xintercept  = field_data_all$lfm_tlp, colour = "blue") 
```
...
```{r}
#Data
field_segment_angio_df <- field_data_all %>% 
...
u_mod_angio_b <- segmented(u_mod_angio, ~lfm)
...
ggplot(aes(y = mpa, 
  ...
  geom_vline(xintercept  = field_segment_angio_df$lfm_tlp, colour = "blue") 
```
...
```{r}
#Data
field_segment_gymno_df <- field_data_all %>% 
...
ggplot(aes(y = mpa, 
  ...
  geom_point()+
  geom_ribbon(aes(xmin = seg_psi_gymno_df$lwr, 
                  xmax = seg_psi_gymno_df$upr), 
              fill = "grey80", 
              color = "grey80")  +
   geom_vline(xintercept  = seg_psi_norand_gymno_df$lfm_psi, color = "red") +
  geom_vline(xintercept  = seg_psi_gymno_df$lfm_psi, linetype = "dotted") +
  geom_vline(xintercept  = field_segment_gymno_df$lfm_tlp, colour = "blue") 
```
