...
anova(m10, m10.5) #don't need interacrtion
...
```{r}
m12 <- lmer(lfm~ water_potential*F.Group*as.factor(year) + (1|plant), data = field_data_all)
...
m12.4<- lmer(lfm~ water_potential*as.factor(year) + water_potential*F.Group + F.Group + (1|plant), data = field_data_all)
summary(m12.4)
...
field_plot <- field_data_all %>% 
 ...
 # geom_smooth(method = "lm", se = F) +
  ...
  color = "black",
  fontface = 'bold', size = 10,
  mapping = aes(x = 9, y = 300, label = label))
