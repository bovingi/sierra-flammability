...
pv_plot <- pv_all_df %>% 
  ...
  labs(title = "PV Curves", 
       ...
       y = "LFM") +
  xlim(0, 10) +
  annotate(geom = "text", x = 10, y = 460, label = 'b', fontface = 'bold', size = 10) +
...
mod_all_long <- mod_all_df %>% 
  select(time_effect, spp_effect, time_spp_effect, spp_AND_time_effects) %>% 
  pivot_longer(cols = c(time_effect, spp_effect, time_spp_effect, spp_AND_time_effects), 
    names_to = "effect") %>% 
  distinct() %>% 
...
mod_all_long_lfm <- mod_all_df %>% 
  select(time_effect, spp_effect, time_spp_effect, spp_AND_time_effects, mpa_effect) %>% 
  pivot_longer(cols = c(time_effect, spp_effect, time_spp_effect, spp_AND_time_effects, mpa_effect), 
    names_to = "effect") %>% 
  distinct() %>% 
  mutate(analysis = case_when(
    effect %in% c("time_effect", "spp_effect", "spp_AND_time_effects", "mpa_effect") ~ "PV Curve", 
    effect %in% c("time_spp_effect") ~ "Full Mod.", 
    TRUE ~ as.character(effect)), 
    decomp = "After Leafout")
mod_all_long_lfm
...
m1 <- lmer(lfm ~ water_potential + month + (1|plant), data =df)
m2 <- lmer(lfm ~ water_potential + (1|plant), data =df)
m3 <- lmer(maxLFM ~ month + (1|plant), data =df)
...
p <- mod_all_df1 %>% 
  ...
  mutate(
    ...
    time_effect = m1 - m2, #week + mpa mod - mpa mod
         ...
         time_mpa_effect = m1, #week + mpa mod 
    mpa_AND_time_effects =  m1 - (time_effect - mpa_effect),
         ) %>% #full minus the effects of mpa and week alone  
  select(time_effect, mpa_effect, time_mpa_effect, mpa_AND_time_effects) %>% 
  pivot_longer(cols = c(time_effect, mpa_effect, time_mpa_effect, mpa_AND_time_effects), 
    names_to = "effect") %>% 
  distinct() %>% 
  mutate(analysis = case_when(
