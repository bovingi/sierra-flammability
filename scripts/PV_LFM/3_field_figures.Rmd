...
ggplot() +
  ...
  geom_vline(aes(xintercept = mean(fig_df$mean_tlp_spp), 
                 #lty = Species
                 ),  size = .5, alpha = .6,
             color = "darkgreen", show.legend = F) +
  tidypaleo::geom_ribbonh(#data = mpa_summs, 
                  aes(xmin = 2, 
                        #mean(mpa_summs$mpa_lwr), 
                  xmax = 3, 
                  x = 2.5),
                    #mean(mpa_summs$mpa_upr)), 
              color = "grey") +
  #geom_hline(aes(yintercept = mean_lfm_fall_spp, lty = Species),
             #size = .5, color = 'darkgreen', alpha = .6, show_guide = F) +
  #stat_smooth(aes(x = LFM, y = predawn, colour = Date, linetype = "Linear Fit"), method = "lm") 
  xlim(-5,0) + 
  labs( y = "Live Fuel Moisture", 
        x = "Midday (-MPa)", 
...
####Setup:
```{r}
field_data_2021 <- field_data_all %>%
 # filter(spp == "ABCO") %>%
  filter(year == 2021) %>% 
  filter(age != "both") %>% 
...
field_data_all %>% 
  ...
  #geom_hline(yintercept = field_data_all$mean_tlp_spp, linetype = "dotted", color = "olivedrab") +
  labs( y ="Predawn (MPa)", 
        x = "Date", 
        #color = "Phenophase", 
        #shape = "Age", 
