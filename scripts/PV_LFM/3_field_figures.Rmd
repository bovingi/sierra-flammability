...
pheno_fig <- pheno_df %>% 
  filter(#category %in% c("Leaves", "Needles"), 
         phenophase_status == "Yes") %>% 
  mutate(lfm = case_when(
    phenophase_name == "Young leaves" ~ 30, 
    phenophase_name == "Young needles" ~ 30, 
    phenophase_name == "Breaking leaf buds" ~ 10,
    phenophase_name == "Breaking needle buds" ~ 10,
    phenophase_name == "Emerging needles" ~ 20, 
    phenophase_name == "Increasing leaf size" ~ 20, 
    phenophase_name == "Leaves" ~ 40, 
    phenophase_name == "Colored leaves" ~ 50,
    #phenophase_name == "Flowers or flower buds" ~ 75, 
   # phenophase_name == "Open flowers" ~ 76,
    phenophase_name == "Pollen cones" ~ 60,
    phenophase_name == "Pollen release" ~ 70, 
   # phenophase_name == "Fruits" ~ 78, 
   # phenophase_name == "Open pollen cones" ~ 86,
   # phenophase_name == "Unripe seed cones" ~ 90,
   # phenophase_name == "Recent fruit of seed drop" ~95,
    phenophase_name == "Ripe fruits" ~ 80,
  #  phenophase_name == "Ripe seed codes" ~ 91
  TRUE ~ NA_real_
  )) %>% 
  mutate(week = week(date)) %>% 
...
Function:
```{r}
pheno.boxplot.vis <- function(species, phenophases, max.y = 350, colors){
...
df3 <- pheno_fig %>%
  filter(spp == "QUKE") %>% 
  filter(phenophase_name %in% c("Breaking leaf buds", "Increasing leaf size"))
...
# Phenophase legend:
df.leg <- pheno_fig %>%
  filter(phenophase_name %in% c("Breaking needle buds", "Young needles", "Emerging needles", "Pollen cones", "Pollen release", "Breaking leaf buds", "Young leaves", "Increasing leaf size"))
df.leg$phenophase_name <- factor(df.leg$phenophase_name, levels = c("Breaking needle buds", "Young needles", "Emerging needles", "Pollen cones", "Pollen release", "Breaking leaf buds", "Young leaves", "Increasing leaf size"))

legend.plot <- ggplot() +
  geom_line(aes(y = lfm, x = date, color = phenophase_name), alpha = 1, 
            data = df.leg, size = 4, position=position_dodge(w=1)) +
  labs(color = "Phenophase") +
  scale_color_manual(values = c("darkblue", "lightgreen", "gray", "brown", "gold", "lightblue", "darkgreen", "orange")) + #Holder color palette, matched to the phenophase vector above
...
```{r}
abco.phenophases <- c("Breaking needle buds", "Young needles")
abco.colors <- c("darkblue", "lightgreen")
p.abco <- pheno.boxplot.vis(species = "ABCO", phenophases = abco.phenophases,
                            colors = abco.colors)

pije.phenophases <- c("Emerging needles", "Young needles")
pije.colors <- c("lightgreen", "gray")
p.pije <- pheno.boxplot.vis(species = "PIJE", phenophases = pije.phenophases,
                            colors = pije.colors)

cade.phenophases <- c("Pollen release", "Pollen cones")
cade.colors <- c("brown", "gold")
p.cade <- pheno.boxplot.vis(species = "CADE", phenophases = cade.phenophases,
                            max.y = 150, colors = cade.colors)

arpa.phenophases <- c("Breaking leaf buds", "Young leaves")
arpa.colors <- c("lightblue", "darkgreen")
p.arpa <-pheno.boxplot.vis(species = "ARPA", phenophases = arpa.phenophases,
                           max.y = 210, colors = arpa.colors)

ceco.phenophases <- c("Young leaves")
ceco.colors <- c("darkgreen")
