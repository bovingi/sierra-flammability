...
Function:
```{r}
pheno.boxplot.vis <- function(species, phenophases, max.y = 350){
...
p3 <- p1 + annotation_custom(ggplotGrob(p2), xmin = "May", xmax = "Oct.",
                       ymin = -10, ymax = 30)
p3
```
...
p.cade <- pheno.boxplot.vis(species = "CADE", phenophases = cade.phenophases, max.y = 150)

arpa.phenophases <- c("Breaking leaf buds", "Young leaves")
p.arpa <-pheno.boxplot.vis(species = "ARPA", phenophases = arpa.phenophases, max.y = 210)

ceco.phenophases <- c("Young leaves")
p.ceco <- pheno.boxplot.vis(species = "CECO", phenophases = ceco.phenophases, max.y = 260)

# Not working
quke.phenophases <- c("Breaking leaf buds", "Increasing leaf size")
p.quke <- pheno.boxplot.vis(species = "QUKE", phenophases = quke.phenophases, max.y = 260)

ggarrange(p.abco, p.pije, p.cade, p.ceco, p.arpa, p.quke, common.legend = T)
```
