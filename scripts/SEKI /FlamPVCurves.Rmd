...
here = here::here
```
swc is from the excel-calculated saturated water content.

```{r}
#getwd()
pressure_volume <- read.csv(here("processed-data", "pressure_volume_data_NOsatwt.csv")) %>%
  clean_names() %>% 
  select(-swc) %>% 
  mutate(water_weight = fresh_weight - dry_weight, 
         ...
         rwd = 100 - rwc) %>% 
  group_by("spp", "sample") %>% 
  mutate(rwc_at_tlp_r = rwc from the row where tlp is equal to tlp_excel) ##need to do this. 
  )) 
  
str(pressure_volume)

#fresh.weight.saturated.excel is 
```

Extrapolate? 
```{r}
pressure_volume %>%
  ggplot(aes(x = water_weight, y = water_potential, color = spp)) +
  geom_point(size = 1) +
  theme_bw()
```
...
pressure_volume %>%
  #filter(spp == "ABCO") %>%
  ggplot(aes(y = RWC, x = lfm, color = spp)) +
  geom_point(size = 1) +
  theme_bw()
```
