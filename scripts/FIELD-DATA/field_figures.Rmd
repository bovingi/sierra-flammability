...
###Field Data
```{r}
field_data_all <- readr::read_csv(here::here("processed-data", "compiled-datasets", "SEKI", "field_data_2020_2021.csv")) %>% 
  clean_names() %>% 
  filter(spp %in% c("ABCO", "CADE","PIJE","ARPA", "CECO", "QUKE")) 

 field_data_all$date <- lubridate::ymd(field_data_all$date) #wont work as part of a pipe for some reason.. 

 field_data_df<- field_data_all %>% 
...
```{r}
pheno_df <- read_csv(here("processed-data", "compiled-datasets", "phenology_czo_2021.csv")) %>% 
...
```{r}
# mean_tlp_allspp <- field_data_all %>% 
#   select(mean_tlp) %>% 
#   drop_na() %>% 
...
p1 <- field_data_df %>% 
 ...
 #annotate("text", x= -1, y= -5, aes(label=strategy), size = 3) +
  ...
  geom_hline(aes(yintercept = mean_tlp), size = 1, 
             color = "#db8872", 
             linetype = "dotted")+
  th +
  labs(y= "", 
...
p2 <- field_data_df %>% 
 ...
 #annotate("text", x= -1, y= -5, aes(label=strategy), size = 3) +
  ...
  ggpubr::stat_regline_equation(size = 4) +
  geom_hline(aes(yintercept = mean_tlp), 
             size = 1, 
             color = "#db8872", 
             linetype = "dotted")+
  th +
...
field_data_df %>% 
 ...
 #annotate("text", x= -1, y= -5, aes(label=strategy), size = 3) +
  ...
  geom_hline(aes(yintercept = mean_tlp), size = .5, color = "olivedrab", type = "dashed")+
  th
```
...
field_data_df %>%
  ...
  geom_point(size = 1) +
  geom_vline(aes(xintercept = mean_tlp), size = .5, color = "olivedrab", linetype = "dotted") +
...
field_data_all %>%
  ...
  geom_point(size = 1) +
  #stat_smooth(aes(x = LFM, y = predawn, colour = Date, linetype = "Linear Fit"), method = "lm") +
  #geom_vline(aes(xintercept = mean_tlp_allspp, size = .5, color = "olivedrab"))+
  xlim(-5,0) + 
  labs( y = "Live Fuel Moisture", 
        x = "Midday (-MPa)", 
        color = "Tissue Age", 
...
field_data_df %>%
  ...
  ggplot(aes(x = midday, y = lfm, color = as.factor(month))) +
  geom_point(size = 0.8, alpha = .6) +
  geom_vline(aes(xintercept = mean_tlp), size = 1, color = "black", linetype = "dotted") +
  #stat_smooth(aes(x = LFM, y = predawn, colour = Date, linetype = "Linear Fit"), method = "lm") 
  xlim(-5,0) + 
  labs( y = "Live Fuel Moisture", 
...
field_data_df %>%
  ...
  geom_vline(aes(xintercept = mean_tlp), size = .5, alpha = .6,
...
field_data_df %>%
  ...
  ggplot(aes(x = midday, 
             ...
             color = age)) +
  geom_point(size = 1, alpha = .7) +
  geom_vline(aes(xintercept = mean(mean_tlp)), size = .5, alpha = .6,
             color = "black", linetype = "dotted") +
  #stat_smooth(aes(x = LFM, y = predawn, colour = Date, linetype = "Linear Fit"), method = "lm") 
  xlim(-5,0) + 
  labs( y = "Live Fuel Moisture", 
...
field_data_all %>%
  ggplot(aes(x = midday, y = lfm, color = month, shape = spp)) +
  geom_point(size = 1) +
  geom_vline(aes(xintercept = mean_tlp), size = .5, 
             color = "olivedrab", 
             linetype = "dotted"
             ) +
  #stat_smooth(aes(x = LFM, y = predawn, colour = Date, linetype = "Linear Fit"), method = "lm") 
...
field_data_2021_longer %>% 
 ggplot() + 
  ...
  #geom_hline(yintercept = mean_tlp_spp, linetype = "dotted", color = "olivedrab") +
  labs( y ="Predawn (MPa)", 
        x = "Date", 
        #color = "Phenophase", 
        #shape = "Age", 
...
field_data_df %>% 
  ...
  #geom_hline(yintercept = field_data_df$mean_tlp, linetype = "dotted", color = "olivedrab") +
  labs( y ="Predawn (MPa)", 
        x = "Date", 
        #color = "Phenophase", 
        #shape = "Age", 
