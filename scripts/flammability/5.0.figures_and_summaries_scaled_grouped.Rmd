...
```
####Prop ignite but boxplots: 
#HERE

11/17/2022 --> releveling is not working, and I can't figure out why....???
...
mem_data_prop_ignite_r <- mem_data_prop_ignite_r %>% 
  mutate(bin_relevel = forcats::fct_relevel(bin, "[10,20]",
                                         ...
                                         "(70,80]", "(80,90]", "(90,100]", "(100,110]",
                                                "(110,120]", "(120,130]", "(130,140]", "(150,160]","(160,170]",
                                                "(170,180]", "(180,190]", "(190,200]", "(230,240]")) %>% 
  arrange() %>% 
...
df <- mem_data_prop_ignite_r %>% 
  ...
  group_by(spp) %>% #group by year, model, spp, group column
  add_tally %>% 
  mutate(total = sum(ignition, na.rm = T), 
         prop_ignite_dry = paste0(round(100 * total/n)), 
         prop_ignite_dry = as.numeric(prop_ignite_dry)) %>% 
 # mutate(prop_dry = ) %>% 
...
## TTI
Gathering effect size from above table and standard error from the summary of the mixed effects model
```{r}
summary(tti_mod)

species <- c("A. concolor", "A. patula", "C. decurrens", "P. jeffreyi", "Q. kelloggii")
tti.ES.lfm <- c(5.611, 6.794, 1.552, -0.898,-2.845) # Following this order: ABCO,  ARPA, CADE, PIJE, QUKE (no CECO)
tti.SE.lfm <- c(3.0378, 4.6098, 5.3613, 4.3614, 7.6662)
tti.sig.lfm <- c('No', 'No', 'No', 'No', 'No')
tti.ES.mpa <- c(-3.335,9.860,9.076, -0.390, 1.751) 
tti.SE.mpa <- c(2.8881, 4.1848, 5.1628, 4.1256, 7.5027)
tti.sig.mpa <- c('No', 'Yes', 'No', 'No', 'No')
...
tti.ES.lfm.plot <- ggplot(data = tti.ES.df, aes(x = species, y = tti.ES.lfm, 
  ...
  labs(y = 'LFM x Species Effect Size on TTI', color = 'Species', shape = 'Significant?') +
  geom_point(size = 5) +
  geom_linerange(aes(ymin = tti.ES.lfm - 0.5*tti.SE.lfm, ymax = tti.ES.lfm + 0.5*tti.SE.lfm)) +
  theme_bw() +
  scale_color_manual(values = c('black', '#BD282F', '#FDD358', '#87B37A', '#5D5D81')) +
...
tti.ES.mpa.plot <- ggplot(data = tti.ES.df, aes(x = species, y = tti.ES.mpa, 
  ...
  labs(y = 'MPa x Species Effect Size on TTI', color = 'Species', shape = 'Significant?') +
...
## FH
For now, just getting the effect sizes in until we decide on what metric to use to show variance
```{r}
summary(fh_mod)

fh.ES.lfm <- c(-1.857, 3.107, -2.255, 0.407,-0.005) # Following this order: ABCO,  ARPA, CADE, PIJE, QUKE (no CECO)
fh.SE.lfm <- c(0.8147, 1.2257, 1.4422, 1.1660, 2.0679)
fh.sig.lfm <- c('Yes', 'Yes', 'No', 'No', 'No')
fh.ES.mpa <- c(-2.126,3.702,0.273, 1.153, 1.978) 
fh.SE.mpa <- c(0.7819, 1.1283, 1.3925, 1.1130, 2.0264)
fh.sig.mpa <- c('Yes', 'Yes', 'No', 'No', 'No')
...
fh.ES.lfm.plot <- ggplot(data = fh.ES.df, aes(x = species, y = fh.ES.lfm, 
  ...
  labs(y = 'LFM x Species Effect Size on FH', color = 'Species', shape = 'Significant?') +
  geom_point(size = 5) +
...
fh.ES.mpa.plot <- ggplot(data = fh.ES.df, aes(x = species, y = fh.ES.mpa, 
  ...
  labs(y = 'MPa x Species Effect Size on FH', color = 'Species', shape = 'Significant?') +
  geom_point(size = 5, cex = 5) +
...
For now, just getting the effect sizes in until we decide on what metric to use to show variance
```{r}
gd.ES.lfm <- c(11.623, -12.034, 2.005, -12.020, -66.120) # Following this order: ABCO,  ARPA, CADE, PIJE, QUKE (no CECO)
gd.sig.lfm <- c('No', 'No', 'No', 'No', 'Yes')
gd.ES.mpa <- c(16.461, -14.780, -1.010, -11.924, -53.761)
gd.sig.mpa <- c('Yes', 'No', 'No', 'No', 'Yes')
gd.ES.df <- data.frame(species, gd.ES.lfm, gd.sig.lfm, gd.ES.mpa, gd.sig.mpa)

gd.ES.lfm.plot <- ggplot(data = gd.ES.df, aes(x = species, y = gd.ES.lfm, 
                                                color = species, shape = gd.sig.lfm)) +
  labs(y = 'LFM x Species Effect Size on GD', color = 'Species', shape = 'Significant?') +
  geom_point(size = 5) +
  theme_bw() +
  scale_color_manual(values = c('black', '#BD282F', '#FDD358', '#87B37A', '#5D5D81')) +
  scale_shape_manual(values = c(16, 10)) +
...
gd.ES.mpa.plot <- ggplot(data = gd.ES.df, aes(x = species, y = gd.ES.mpa, 
  ...
  labs(y = 'MPa x Species Effect Size on GD', color = 'Species', shape = 'Significant?') +
  geom_point(size = 5, cex = 5) +
  theme_bw() +
  scale_color_manual(values = c('black', '#BD282F', '#FDD358', '#87B37A', '#5D5D81')) +
  scale_shape_manual(values = c(16, 10)) +
...
prop_ignite.ES.df <- data.frame(species, prop_ignite.ES.lfm, prop_ignite.sig.lfm, prop_ignite.ES.mpa, prop_ignite.sig.mpa)
...
prop_ignite.ES.lfm.plot <- ggplot(data = prop_ignite.ES.df, aes(x = species, y = prop_ignite.ES.lfm, 
  ...
  labs(y = 'LFM x Species Effect Size on Ignition Rate', color = 'Species', shape = 'Significant?') +
  geom_point(size = 5) +
  theme_bw() +
  scale_color_manual(values = c('black', '#BD282F', '#FDD358', '#87B37A', '#5D5D81')) +
  scale_shape_manual(values = c(16, 10)) +
...
prop_ignite.ES.mpa.plot <- ggplot(data = prop_ignite.ES.df, aes(x = species, y = prop_ignite.ES.mpa, 
  ...
  labs(y = 'MPa x Species Effect Size on Ignition Rate', color = 'Species', shape = 'Significant?') +
  geom_point(size = 5, cex = 5) +
  theme_bw() +
  scale_color_manual(values = c('black', '#BD282F', '#FDD358', '#87B37A', '#5D5D81')) +
  scale_shape_manual(values = c(16, 10)) +
