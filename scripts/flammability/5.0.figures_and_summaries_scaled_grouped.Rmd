...
tti_plot <- mem_data_all_r %>% 
  ...
  scale_color_manual(values = c('black', '#BD282F', '#FDD358', '#87B37A', '#5D5D81')) + # For color by species w/o CECO
...
tti_plot_lfm_r <- mem_data_all_r %>% 
  ...
  #scale_color_manual(values = c('#FDD358', 'black')) + # For color by angio vs gymno; change Species > F.Group (version 2)
  theme(legend.position = "none",
...
tti.ES.lfm <- c(7.4, 9.3, -0.057, 6.4,0.55) # Following this order: ABCO,  ARPA, CADE, PIJE, QUKE (no CECO)
tti.SE.lfm <- c(3.0378, 4.6098, 5.3613, 4.3614, 7.6662)
tti.sig.lfm <- c('No', 'No', 'No', 'No', 'No')
tti.ES.mpa <- c(-3.8,1.7,3.5, -3.4, 1.9) 
...
fh.ES.lfm <- c(-1.7, -1.2, -4.5, -2.3,-3.7) # Following this order: ABCO,  ARPA, CADE, PIJE, QUKE (no CECO)
fh.SE.lfm <- c(0.8147, 1.2257, 1.4422, 1.1660, 2.0679)
fh.sig.lfm <- c('Yes', 'Yes', 'No', 'No', 'No')
fh.ES.mpa <- c(-2.2,-0.4,0.048, -1.1, -0.13) 
...
gd.ES.lfm <- c(16, 16, 18, 12, -4.7) # Following this order: ABCO,  ARPA, CADE, PIJE, QUKE (no CECO)
gd.sig.lfm <- c('No', 'No', 'No', 'No', 'Yes')
gd.SE.lfm <- c(8.163, 11.799, 14.544, 11.419, 21.231)
gd.ES.mpa <- c(16, 8.6, 11, 9.5, 27)
