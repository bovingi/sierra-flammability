...
mem_data_all_raw <- read_csv(here("processed-data", "analysis 2.0", "sierra_flam_data_all.csv")) %>% 
  select(lfm, mpa, fh, fh, fd, gd,tti, prop_ignite, temp_change, ignition, sample_wt, dry_wt, fresh_wt, water_wt, location, site, year_month, spp, individual, bins10lfm, bins5lfm, bins20lfm, notes_flam) %>% 
...
tti_plot_lfm_r <- mem_data_all_r %>%
  ggplot(aes(y = r_tti_lfm, 
             x = lfm_scaled, 
             group = F.Group)) +
  geom_point(alpha= .4, aes(color = F.Group, shape = F.tti.sig))+
...
fh_plot_lfm_r <- mem_data_all_r %>% 
  ggplot(aes(y = r_fh_lfm, 
             x = lfm_scaled, 
             group = F.Group)) +
  geom_point(alpha= .4, aes(color = F.Group, shape = F.fh.sig))+
  geom_smooth(method = "lm", se = F, aes(color = F.Group), alpha = .75) + 
  #color_many2  + # For color by F.Group w/ CECO
  #color_noceco+ # For color by F.Group w/o CECO
...
gd_plot_lfm_r <- mem_data_all_r %>% 
  ggplot(aes(y = r_gd_lfm, 
             x = lfm_scaled, 
             group = F.Group)) +
  geom_point(alpha= .4, aes(color = F.Group, shape = F.gd.lfm.sig))+
  geom_smooth(method = "lm", se = F, aes(color = F.Group), alpha = .75) +
  theme(legend.position = "none",
                       axis.text = element_text(size = 13),
                       axis.title = element_text(size = 16, face = 'bold'))+
...
plot_legend.scatterplot <- mem_data_all_r %>% 
  ...
  geom_smooth(se = F, method = 'lm', aes(color = F.Group)) +
...
scatterplots <- cowplot::plot_grid(blank_plot, tti_plot_lfm_r, fh_plot_lfm_r, gd_plot_lfm_r,
                                   ncol = 1, rel_heights = c(0.9, 10, 9.95, 10))
...
legend_and_boxplot <- cowplot::plot_grid(blank_plot, boxplot.no.tlp.split, blank_plot, legend, ncol = 1, rel_heights = c(0.3,4,1.5,4))
