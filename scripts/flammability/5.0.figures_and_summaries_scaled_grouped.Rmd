...
fh_plot_lfm_r <- mem_data_all_r %>% 
  ...
  labs(x = "", 
       y = "Flame Height") +
  annotate(geom = "text", x = -2.6, y = 57, label = 'd', fontface = 'bold', size = 10)
fh_plot_lfm_r
...
gd_plot_lfm_r <- mem_data_all_r %>% 
  ...
  labs(x = "LFM", 
       y = "Glow Duration") +
  annotate(geom = "text", x = -2.6, y = 450, label = 'g', fontface = 'bold', size = 10)
gd_plot_lfm_r
...
tti.ES.lfm.plot <- ggplot(data = tti.ES.df, aes(x = species, y = tti.ES.lfm, 
  ...
  theme_bw() +
  scale_y_continuous(limits = c(-12.5, 12.5), breaks = c(-10,-5, 0, 5, 10)) +
...
fh.ES.lfm.plot <- ggplot(data = fh.ES.df, aes(x = species, y = fh.ES.lfm, 
  ...
  scale_color_manual(values = c('black', '#BD282F', '#FDD358', '#87B37A', '#5D5D81')) +
  scale_shape_manual(values = c(16, 10)) +
  annotate(geom = "text", x = 0.9, y = 4, label = 'e', fontface = 'bold', size = 10) +
  theme(axis.title.x = element_blank(), axis.text.x = element_blank(),
        axis.title.y = element_blank(), 
        axis.text.y = element_blank(),
        axis.ticks.y = element_blank(),
...
fh.ES.mpa.plot <- ggplot(data = fh.ES.df, aes(x = species, y = fh.ES.mpa,
  ...
  scale_color_manual(values = c('black', '#BD282F', '#FDD358', '#87B37A', '#5D5D81')) +
  scale_shape_manual(values = c(16, 10)) +
  annotate(geom = "text", x = 0.9, y = 4, label = 'f', fontface = 'bold', size = 10) +
  theme(axis.title.x = element_blank(), axis.text.x = element_blank(),
        axis.ticks.y.left = element_blank(), 
        axis.text.y.left = element_blank(),
        axis.title.y.right = element_text(face = 'bold', size = 14),
...
gd.SE.lfm <- c(8.163, 11.799, 14.544, 11.419, 21.231)
gd.ES.mpa <- c(16.461, -14.780, -1.010, -11.924, -53.761)
gd.sig.mpa <- c('Yes', 'No', 'No', 'No', 'Yes')
gd.SE.mpa <- c(8.246, 11.692, 14.276, 11.483, 20.943)
...
gd.ES.lfm.plot <- ggplot(data = gd.ES.df, aes(x = species, y = gd.ES.lfm, 
  ...
  geom_abline(intercept = 0, slope = 0, color = 'black', alpha = 0.5, size = 0.3) +
  theme_bw() +
  scale_y_continuous(limits = c(-80, 40), breaks = c(-75,-50, -25, 0, 25)) +
  scale_color_manual(values = c('black', '#BD282F', '#FDD358', '#87B37A', '#5D5D81')) +
  scale_shape_manual(values = c(16, 10)) +
...
plot_legend2 <- mem_data_all_r %>% 
  ggplot(aes(x = tti, y = mpa, group = Species)) +
  geom_point(aes(color = Species, shape = fh.mpa.sig), size = 2.2) +
  geom_smooth(se = F, method = 'lm', aes(color = Species, lty = fh.mpa.sig)) +
  color_many2  +
  labs(color = "Species", lty = "Significant?", 
       shape = "Significant? ") +
...
main_legend <- cowplot::get_legend(plot_legend2)
```
...
main_figure_no_legend <- cowplot::plot_grid(scatterplots, ES.plots, ncol = 2, rel_widths = c(10,13))
main_figure_no_legend

main_figure <- cowplot::plot_grid(main_figure_no_legend, main_legend, ncol = 2, rel_widths = c(5,1))
main_figure
