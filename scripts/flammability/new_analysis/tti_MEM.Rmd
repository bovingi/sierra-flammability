...
```{r}
topmods <- MuMIn::dredge(lmer(tti ~ lfm_scaled:spp + mpa_scaled:spp + site + year_month + sample_wt_scaled + (1 | individual), data = mem_data_all_dredge, na.action = "na.fail", REML = F))

topmods

top_tti <- lmer(tti ~ lfm_scaled:spp + mpa_scaled:spp + mpa_scaled + lfm_scaled + site + sample_wt_scaled + (1 | individual), data = mem_data_all_dredge, na.action = "na.fail", REML = F)

top_tti_nomains <- lmer(tti ~ lfm_scaled:spp + mpa_scaled:spp + sample_wt_scaled  + (1 | individual), data = mem_data_all_dredge, na.action = "na.fail", REML = F)

aictab(c(top_tti, top_tti_nomains))
...
```{r}
topmods <- MuMIn::dredge(lmer(tti ~ lfm_scaled:spp + lfm_scaled + site + year_month + sample_wt_scaled + (1 | individual), data = mem_data_all_dredge, na.action = "na.fail", REML = F))
...
top_lfm <- lmer(tti ~ lfm_scaled:spp + site + sample_wt_scaled + (1 | individual), data = mem_data_all_dredge, na.action = "na.fail", REML = T)

summary(top_lfm)
```

```{r}
topmods <- MuMIn::dredge(lmer(tti ~ mpa_scaled:spp + mpa_scaled + site + year_month + sample_wt_scaled + (1 | individual), data = mem_data_all_dredge, na.action = "na.fail", REML = F))

topmods

top_mpa <- lmer(tti ~ mpa_scaled:spp + mpa_scaled + site + sample_wt_scaled + (1 | individual), data = mem_data_all_dredge, na.action = "na.fail", REML = T)
...
```{r}
tti_mod_lfm <- lmer(tti ~ lfm_scaled:spp + site + sample_wt_scaled + (1 | individual), data = mem_data_all_dredge, na.action = "na.fail", REML = T)
...
tti.ES.lfm <- c(ref, ref + tti_mod.coef.df1$estimate[7],
...
tti.SE.lfm <- c(tti_mod.coef.df1$std.error[c(4,7,6,5,6)])

tti.sig.lfm <- significance(tti.p.value.lfm, tti.sig.lfm) # Gathering p-values from vectors made in 2.5 section

#mpa: 
ref2 <- tti_mod.coef.df$estimate[2]

tti.ES.mpa <- c(ref2, ref2 + tti_mod.coef.df2$estimate[7],
...
tti.ES.mpa.plot <- ES.plot(tti.ES.df, tti.ES.mpa, tti.sig.mpa, tti.SE.mpa,
                           ...
                           limits.vector = c(0, 25),
                           breaks.vector = c(-5, 0, 5, 10, 15, 20, 25)) +
...
topmods <- MuMIn::dredge(lmer(fh ~ lfm_scaled:spp + mpa_scaled:spp + year_month + sample_wt_scaled + site + (1 | individual), data = mem_data_all_dredge, na.action = "na.fail", REML = F))

topmods

top_ <- lmer(tti ~ lfm_scaled:spp + mpa_scaled:spp + mpa_scaled + lfm_scaled + sample_wt_scaled + site + (1 | individual), data = mem_data_all_dredge, na.action = "na.fail", REML = F)

top_tti_nomains <- lmer(tti ~ lfm_scaled:spp + mpa_scaled:spp + sample_wt_scaled + site + (1 | individual), data = mem_data_all_dredge, na.action = "na.fail", REML = F)
