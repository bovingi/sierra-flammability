...
```
Next: 
Can take best within 6, and aim is for the simplist model. 
...
summary(top_lfm_tti)
performance::multicollinearity(top_lfm_tti)
...
null_tti <- lmer(tti ~  spp + sample_wt_scaled + year_month + (1 | individual), data = mem_data_all, REML = F)

df_tti1 <- as.data.frame(aictab(c(top_lfm_tti, top_lfm_int_tti, top_mpa_noint_tti, top_mpa_int_tti, null_tti)))
...
```{r}
tti_mod.coef.df1 <- broom.mixed::tidy(top_lfm_int_tti) %>% 
...
#tti.sig.lfm <- (c("Yes", "No", "Yes", "Yes", "Yes"))
...
tti.ES.mpa <- c(ref2, 
                ...
                ref2 + tti_mod.coef.df2$estimate[12]) 
tti.SE.mpa <- c(tti_mod.coef.df2$std.error[c(2,11,10,9,12)])

ref <- tti_mod.coef.df2$p.value[2]

tti.p.value.mpa <- c(ref, #Abco
                tti_mod.coef.df2$p.value[11], #pije
                tti_mod.coef.df2$p.value[10],
                tti_mod.coef.df2$p.value[9],
                tti_mod.coef.df2$p.value[12])

#tti.sig.mpa <- (c("Yes", "No", "Yes", "Yes", "Yes"))
tti.sig.mpa <- significance(tti.p.value.lfm, tti.sig.lfm) # Gathering p-values from vectors made in 2.5 section

tti.ES.df <- data.frame(
  species,
  tti.ES.lfm,
  tti.SE.lfm,
  tti.p.value.lfm,
...
dotplot <- cowplot::plot_grid(ES_legend, dotplot.prop.ig, 
                              nrow = 2, 
                              rel_heights = c(1,1))
dotplot

tti_plots <- cowplot::plot_grid(tti_plots1, ES_legend, 
                                ncol = 2, 
                                rel_widths = c(1, .2))
tti_plots
...
ggsave(plot = tti_plots, bg = 'white', 
       here("figures", "main-figures", "Fig2.tti.vs.hydration_nodotplot.jpg"),
       width = 16, height = 8)
```
...
null_fh <- lmer(fh ~ spp + sample_wt_scaled + year_month +(1 | individual), data = mem_data_all, REML = F)

df_fh1 <- as.data.frame(aictab(c(top_lfm_noint_fh, top_lfm_int_fh, top_mpa_noint_fh, top_mpa_int_fh, null_fh)))
...
top_lfm_fd <- lmer(fd ~ spp + lfm_scaled + sample_wt_scaled + (1|individual), data = mem_data_all, REML = F)
summary(top_lfm_fd)
...
null_fd <- lmer(fd ~ spp + sample_wt_scaled + year_month + (1|individual), data = mem_data_all, REML = F)

top_mpa_int_fd <- lmer(fd ~ sample_wt_scaled + year_month + mpa_scaled*spp + (1|individual), data = mem_data_all, REML = F)
#summary(top_mpa_fd)

top_mpa_fd <- lmer(fd ~ sample_wt_scaled + year_month + mpa_scaled + spp + (1|individual), data = mem_data_all, REML = F)

top_mpa_noint_fd <- lmer(fd ~ sample_wt_scaled + year_month + spp + mpa_scaled + (1|individual), data = mem_data_all, REML = F)

null_fd <- lmer(fd ~ sample_wt_scaled + year_month + spp + (1|individual), data = mem_data_all, REML = F)

df_fd1 <- as.data.frame(aictab(c(top_lfm_fd, top_lfm_int_fd, null_fd, top_mpa_int_fd, top_mpa_noint_fd)))
df_fd1
...
null_temp<- lmer(temp_change ~ spp + sample_wt_scaled + year_month +(1|individual) + (1|site), data = mem_data_all, REML = F)

df_temp1 <- as.data.frame(aictab(c(top_lfm_temp, top_lfm_int_temp, top_mpa_noint_temp, top_mpa_int_temp, null_temp)))
df_temp1
...
null_gd <- lmer(gd ~  year_month +sample_wt_scaled + spp + (1|individual), data = mem_data_all, REML = F)
