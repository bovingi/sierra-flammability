...
####Models: 
```{r}
tti_mod <- lmer(tti ~ lfm_scaled*spp + mpa_scaled*spp + (1 | individual), data = angio_df, REML = F)
fh_mod <- lmer(fh ~ lfm_scaled*spp + mpa_scaled*spp  + (1 | individual), data = angio_df, REML = F)
tti_mod <- lmer(tti ~ lfm_scaled*spp + mpa_scaled*spp  + (1 | individual), data = angio_df, REML = F)

tti_mod <- lmer(tti ~ lfm_scaled*spp + mpa_scaled*spp  + (1 | individual), data = gymno_df, REML = F)
fh_mod <- lmer(fh ~ lfm_scaled*spp + mpa_scaled*spp  + (1 | individual), data = gymno_df, REML = F)
gd_mod <- lmer(gd ~ lfm_scaled*spp + mpa_scaled*spp  + (1 | individual), data = gymno_df, REML = F)
```

####Extracting Coefficients: 
```{r}
tti_mod.coef.df <- tidy(tti_mod) %>% 
