...
mem_data_all_dredge <- mem_data_all %>% 
  drop_na(lfm_scaled, spp, mpa_scaled, year_month, sample_wt_scaled, site, tti)
```


```{r}
#check for collinearity: 
noints <- lmer(tti ~ lfm_scaled + mpa_scaled + spp + sample_wt_scaled  + (1 | individual), data = mem_data_all_dredge, na.action = "na.fail", REML = F)

performance::multicollinearity(noints)
```

Mpa and TTI are moderately collinear (vif > 2), so will be considered in separate models:

```{r}
topmods.lfm <- MuMIn::dredge(lmer(tti ~ lfm_scaled:spp + lfm_scaled + site + sample_wt_scaled + year_month + (1 | individual), data = mem_data_all_dredge, na.action = "na.fail", REML = F)) 
...
top_lfm <- lmer(tti ~ lfm_scaled:spp + lfm_scaled + sample_wt_scaled + (1 | individual), data = mem_data_all_dredge, na.action = "na.fail", REML = T)
...
```{r}
topmods.mpa <- MuMIn::dredge(lmer(tti ~ mpa_scaled:spp + mpa_scaled + site + year_month + sample_wt_scaled + (1 | individual), data = mem_data_all_dredge, na.action = "na.fail", REML = F))

topmods.mpa

top_mpa <- lmer(tti ~ mpa_scaled:spp + mpa_scaled + sample_wt_scaled + (1 | individual), data = mem_data_all_dredge, na.action = "na.fail", REML = T)
...
```{r}
tti_mod.coef.df1 <- tidy(top_lfm) %>% 
  select(term, estimate, std.error, p.value)%>% 
  mutate(metric = 'TTI') %>% 
  mutate(pred = 'lfm')


tti_mod.coef.df2 <- tidy(top_mpa) %>% 
  select(term, estimate, std.error, p.value)%>% 
  mutate(metric = 'TTI') %>% 
  mutate(pred = 'mpa')
...
tti.ES.lfm <- c(ref, #Abco
                ...
                ref + tti_mod.coef.df1$estimate[5],
                ref + tti_mod.coef.df1$estimate[4],
                ref + tti_mod.coef.df1$estimate[7])
  # Following this order: ABCO, PIJE, CADE, ARPA, QUKE (no CECO)

tti.SE.lfm <- c(tti_mod.coef.df1$std.error[c(2,7,6,5,6)])
...
tti.SE.mpa <- c(tti_mod.coef.df2$std.error[c(2,7,6,5,8)])
...
ggplot(aes(y = tti, x = lfm_scaled, color = Species)) +
  ...
  theme(#axis.title.x = element_blank(), 
             ...
             # panel.grid = element_blank(), axis.ticks.x = element_blank(),
              ...
              plot.margin = unit(c(.8, 0.05, 0.2, 0.1), 'cm'),
              legend.position = 'none')  +
  annotate(geom = "text", x = -2.5, y = 159, label = 'c',
           fontface = 'bold', size = 10) 
  tti_scatterplot
  ...
  theme(#axis.title.x = element_blank(),
            ...
            #  axis.title.y.right = element_text(face = 'bold', size = 14),
             ...
             # panel.grid = element_blank(), axis.ticks.x = element_blank(),
              ...
              plot.margin = unit(c(.8, 0.05, 0.2, 0.1), 'cm'),
              legend.position = 'none')  +
  annotate(geom = "text", x = -3, y = 159, label = 'd',
           fontface = 'bold', size = 10) 
  tti_scatterplot.mpa
...
```{r}
mem_data_all_dredge <- mem_data_all %>% 
  drop_na(lfm_scaled, spp, mpa_scaled, year_month, sample_wt_scaled, site, fh
          )

topmods.lfm <- MuMIn::dredge(lmer(fh ~ lfm_scaled:spp + lfm_scaled + year_month + sample_wt_scaled + site + (1 | individual), data = mem_data_all_dredge, na.action = "na.fail", REML = F))
topmods.lfm

summary(model.avg(topmods.lfm, subset = delta < 2))

topmods.mpa <- MuMIn::dredge(lmer(fh ~ mpa_scaled:spp + mpa_scaled + year_month + sample_wt_scaled + site + (1 | individual), data = mem_data_all_dredge, na.action = "na.fail", REML = F))
topmods.mpa
...
```{r}
mem_data_all_dredge <- mem_data_all %>% 
  drop_na(lfm_scaled, spp, mpa_scaled, year_month, sample_wt_scaled, site, fd
          )

topmods.lfm <- MuMIn::dredge(lmer(fd ~ lfm_scaled:spp + lfm_scaled + year_month + sample_wt_scaled + site + (1 | individual), data = mem_data_all_dredge, na.action = "na.fail", REML = F))
topmods.lfm


topmods.mpa <- MuMIn::dredge(lmer(fd ~ mpa_scaled:spp + mpa_scaled + year_month + sample_wt_scaled + site + (1 | individual), data = mem_data_all_dredge, na.action = "na.fail", REML = F))
topmods.mpa
...
```{r}
mem_data_all_dredge <- mem_data_all %>% 
  drop_na(lfm_scaled, spp, mpa_scaled, year_month, sample_wt_scaled, site, temp_change
          )

topmods.lfm <- MuMIn::dredge(lmer(temp_change ~ lfm_scaled:spp + lfm_scaled + year_month + sample_wt_scaled + site + (1 | individual), data = mem_data_all_dredge, na.action = "na.fail", REML = F))
topmods.lfm


topmods.mpa <- MuMIn::dredge(lmer(temp_change ~ mpa_scaled:spp + mpa_scaled + year_month + sample_wt_scaled + site + (1 | individual), data = mem_data_all_dredge, na.action = "na.fail", REML = F))
topmods.mpa
...
```{r}
mem_data_all_dredge <- mem_data_all %>% 
  drop_na(lfm_scaled, spp, mpa_scaled, year_month, sample_wt_scaled, site, gd
          )

topmods.lfm <- MuMIn::dredge(lmer(gd ~ lfm_scaled:spp + lfm_scaled + sample_wt_scaled + site + (1 | individual), data = mem_data_all_dredge, na.action = "na.fail", REML = F))
topmods.lfm


topmods.mpa <- MuMIn::dredge(lmer(gd ~ mpa_scaled:spp + mpa_scaled + sample_wt_scaled + site + (1 | individual), data = mem_data_all_dredge, na.action = "na.fail", REML = F))
topmods.mpa
