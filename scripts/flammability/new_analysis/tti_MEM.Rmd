...
tti_mod.coef.df2 <- tidy(top_mpa_tti) %>% 
 select(term, estimate, std.error, p.value)%>% 
  mutate(metric = 'TTI') %>% 
  mutate(pred = 'mpa')
...
# tti_mod.coef.df2 %>% 
#   dplyr::filter(term %in% c("lfm_scaled",
#                          "lfm_scaled:spppije",
#                          "lfm_scaled:sppcade",
#                          "lfm_scaled:spparpa",
#                          "lfm_scaled:sppquke")) %>%
...
tti.p.value.lfm <- c(ref, #Abco
                ref + tti_mod.coef.df1$p.value[11], #pije
                ref + tti_mod.coef.df1$p.value[10],
                ref + tti_mod.coef.df1$p.value[9],
                ref + tti_mod.coef.df1$p.value[8])
...
tti.SE.mpa <- c(tti_mod.coef.df2$std.error[c(6,10,9,8,11)])

ref <- tti_mod.coef.df2$p.value[6]
tti.p.value.lfm <- c(ref, #Abco
                ref + tti_mod.coef.df2$p.value[10], #pije
                ref + tti_mod.coef.df2$p.value[9],
                ref + tti_mod.coef.df2$p.value[8],
                ref + tti_mod.coef.df2$p.value[11])
...
df_fh1 <- as.data.frame(aictab(c(top_lfm_noint_fh, top_lfm_int_fh, top_mpa_noint_fh, top_mpa_int_fh)))
...
df_temp1 <- as.data.frame(aictab(c(top_lfm_temp, top_lfm_int_temp, top_mpa_temp, top_mpa_int_temp)))
df_temp1
...
tab_model(top_mpa_tti, top_mpa_int_tti, top_lfm_tti, top_lfm_int_tti,
          show.reflvl = FALSE, 
          digits = 3, 
          show.aic = TRUE, 
          show.ci = FALSE,
