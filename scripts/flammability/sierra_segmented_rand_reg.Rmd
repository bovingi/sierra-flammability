...
```{r} 
seg_data_subset %>% group_split(spp) %>%
   setNames(unique(seg_data_subset$spp))  %>%
...
```{r}
seg_abco_s20_subset <- ABCO %>% 
  filter(year_month == "2020_September")

seg_abco_o20_subset <- ABCO %>% 
  filter(year_month == "2020_October")

seg_arpa_s20_subset <- ARPA %>% 
  filter(year_month == "2020_September")

seg_arpa_o20_subset <- ARPA %>% 
  filter(year_month == "2020_October")

seg_cade_s20_subset <- CADE %>% 
  filter(year_month == "2020_September")

seg_cade_o20_subset <- CADE %>% 
  filter(year_month == "2020_October")

seg_ceco_s20_subset <- CECO %>% 
  filter(year_month == "2020_September")

seg_ceco_o20_subset <- CECO %>% 
  filter(year_month == "2020_October")

seg_pije_s20_subset <- PIJE %>% 
  filter(year_month == "2020_September")

seg_pije_o20_subset <- PIJE %>% 
  filter(year_month == "2020_October")

seg_quke_s20_subset <- QUKE %>% 
  filter(year_month == "2020_September")

seg_quke_o20_subset <- QUKE %>% 
  filter(year_month == "2020_October")
...
seg_data_subset %>% 
  ...
  summarise(max_lfm = max(lfm), 
            max_mpa = max(mpa)) %>% 
  group_split(spp) %>% 
  setNames(paste0('max_', unique(seg_data_subset$spp)))  %>%
  list2env(envir = globalenv())
