...
mem_data_all_6 <- read_csv(here("processed-data", "analysis 2.0", "sierra_flam_data_all.csv"), show_col_types = F) %>% 
  ...
  drop_na(mpa_scaled, lfm_scaled, sample_wt_scaled) %>% 
   mutate(spp = toupper(spp), 
          spp = fct_relevel(spp, "ABCO","PIJE","CADE","ARPA", "CECO", "QUKE")) 
```
...
ggplot() +
  geom_point(aes(y = pred, 
                 x = date, 
                 color = spp)) +
  geom_smooth(aes(y = pred, x = date, color = spp), se = F)+
  labs(color = "Species") +
...
plots <- cowplot::plot_grid(tti_pred, fh_pred, temp_pred,temp_max_pred,
                                   ncol = 2
                                   , rel_heights = c(1, 1, 1,1)
                                   )

legend_plots <- cowplot::plot_grid(plots, legend, ncol = 2, rel_widths = c(6,.5))
legend_plots
```
