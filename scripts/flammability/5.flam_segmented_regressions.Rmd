...
#Mpa vs. Flammability Segmented Regressions
...
predictors <- c("tti", "fh", "fd", "gd", "temp_change", "prop_ignite", "lower_temp_max")
groups <- c("Angiosperm", "Gymnosperm")

# Function to fit models and extract results
run_changepoint_analysis <- function(data, predictor, group_label) {
  df <- data %>%
    filter(F.Group == group_label) %>%
    drop_na(mpa, .data[[predictor]]) %>%
    mutate(id = individual)

  ## Random effects model
  rand_mod <- tryCatch(
    lme(reformulate("mpa + spp", response = predictor), random = ~1 | id, data = df),
    error = function(e) NULL
  )

  rand_seg <- tryCatch(
    segmented.default(rand_mod, ~mpa, npsi = list(mpa = 1)),
    error = function(e) NULL
  )

  rand_result <- if (!is.null(rand_seg) && inherits(rand_seg, "segmented")) {
    tibble(
      F.Group = group_label,
      model_type = "random",
      predictor = predictor,
      changepoint = summary(rand_seg)$psi[2],
      se = summary(rand_seg)$psi[3],
      lwr = changepoint - se / 2,
      upr = changepoint + se / 2,
      p_value = NA_real_
    )
  } else {
    NULL
  ...
  lm_mod <- tryCatch(
    lm(reformulate("mpa + spp", response = predictor), data = df),
    error = function(e) NULL
  )

  seg_mod <- tryCatch(
    segmented(lm_mod, ~mpa, npsi = 1),
    error = function(e) NULL
  )

  davies_p <- tryCatch(
    davies.test(seg_mod)$p.value,
    error = function(e) NA_real_
  )

  lm_result <- if (!is.null(seg_mod) && inherits(seg_mod, "segmented")) {
    tibble(
      F.Group = group_label,
      model_type = "no_random",
      predictor = predictor,
      changepoint = summary(seg_mod)$psi[2],
      se = summary(seg_mod)$psi[3],
      lwr = changepoint - se / 2,
      upr = changepoint + se / 2,
      p_value = davies_p
    )
  ...
  bind_rows(rand_result, lm_result)
}

# Run for all predictor × group combinations
changepoint_results <- tidyr::expand_grid(predictor = predictors, group = groups) %>%
  mutate(results = map2(predictor, group, ~run_changepoint_analysis(mem_data_all, .x, .y))) %>%
  unnest(results, names_repair = "unique")  # prevents column name conflict
...
#MPa vs. LFM Segmented Regressions
...
field_segment_angio_df <- pv_all_df_4 %>% 
  ...
  #drop_na(mpa, lfm) %>% 
  mutate(id = plant) %>% 
  filter(mpa < -0.5) #remove the plateau effect area
 # filter(lfm > 80, 
       #  lfm < 150)
...
field_segment_gymno_df <- pv_all_df_4 %>% 
  ...
  mutate(id = plant,
         mpa = water_potential) %>% 
  drop_na(mpa, lfm) %>% 
  filter(mpa < -0.5) #remove plateau effect area
