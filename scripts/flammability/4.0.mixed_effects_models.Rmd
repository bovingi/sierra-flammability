...
mem_data_all <- read_csv(here("processed-data", "analysis 2.0", "sierra_flam_data_all.csv")) %>% 
  ...
  mutate(excess_water_scaled = scale(excess_water))
```
...
## Site, species, year_month, sample_wt
```{r}
mpa_mod_fig.mpa <- lmer(tti ~ mpa + spp + site + year_month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(mpa_mod_fig.mpa)
```

```{r}
max_model <- lmer(tti ~ spp + dw_flam_sample + ww_flam_sample + mpa + lfm+ excess_water_scaled + site + year_month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(max_model)
```

```{r}
max_model_nolfm <- lmer(tti ~ spp + dw_flam_sample + ww_flam_sample + mpa + excess_water_scaled + site +  year_month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(max_model_nolfm)
```

```{r}
max_model_nompa <- lmer(tti ~ spp + dw_flam_sample + ww_flam_sample + lfm+ excess_water_scaled + site +  year_month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(max_model_nompa)
```

```{r}
m3 <- lmer(tti ~ spp + ww_flam_sample + site + year_month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m3)
```

```{r}
m6 <- lmer(tti ~ dw_flam_sample + spp + site + year_month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m6)
```

```{r}
m6.5 <- lmer(tti ~ dw_flam_sample + ww_flam_sample + spp + site + year_month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m6.5)
```

```{r}
m7 <- lmer(tti ~ mpa + lfm+ spp + site + year_month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m7)
```

```{r}
m10 <- lmer(tti ~ lfm+ spp + site + year_month + sample_wt + (1 | individual), data = mem_data_all)
...
```{r}
m11 <- lmer(tti ~ mpa*spp + site + year_month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m11)
```

```{r}
m12 <- lmer(tti ~ excess_water_scaled + spp + site + year_month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m12)
```

```{r}
m12.5 <- lmer(tti ~ excess_water_scaled + mpa + lfm+ spp + site + year_month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m12.5) ##High correlation! Don't use
```

```{r}
m13 <- lmer(tti ~ excess_water_scaled + lfm+ spp + site + year_month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m13) ##High correlation! Don't use
```

```{r}
m14 <- lmer(tti ~ excess_water_scaled + spp + site + year_month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m14)
```

```{r}
m15 <- lmer(tti ~ excess_water_scaled * spp + site + year_month + sample_wt + (1 | individual), data = mem_data_all)
...
```{r}
max_model <- lmer(tti ~ spp + dw_flam_sample + ww_flam_sample + mpa + lfm+ excess_water_scaled + site + year_month + (1 | individual), data = mem_data_all)

performance::multicollinearity(max_model)
```

```{r}
max_model_nolfm <- lmer(tti ~ spp + dw_flam_sample + ww_flam_sample + mpa + excess_water_scaled + site +  year_month + (1 | individual), data = mem_data_all)

performance::multicollinearity(max_model_nolfm)
```

```{r}
max_model_nompa <- lmer(tti ~ spp + dw_flam_sample + ww_flam_sample + lfm+ excess_water_scaled + site +  year_month + (1 | individual), data = mem_data_all)

performance::multicollinearity(max_model_nompa)
```

```{r}
m3 <- lmer(tti ~ spp + ww_flam_sample + site + year_month + (1 | individual), data = mem_data_all)

performance::multicollinearity(m3)
```

```{r}
m6 <- lmer(tti ~ dw_flam_sample + spp + site + year_month + (1 | individual), data = mem_data_all)
...
```{r}
m6.5 <- lmer(tti ~ dw_flam_sample + ww_flam_sample + spp + site + year_month + (1 | individual), data = mem_data_all)

performance::multicollinearity(m6.5)
```

```{r}
m7 <- lmer(tti ~ mpa + lfm+ spp + site + year_month + (1 | individual), data = mem_data_all)

performance::multicollinearity(m7)
```

```{r}
m10 <- lmer(tti ~ lfm+ spp + site + year_month + (1 | individual), data = mem_data_all)
...
```{r}
m11 <- lmer(tti ~ mpa*spp + site + year_month + (1 | individual), data = mem_data_all)
...
```{r}
m12.5 <- lmer(tti ~ dw_flam_sample + mpa + lfm+ spp + site + year_month + (1 | individual), data = mem_data_all)
...
```{r}
m13 <- lmer(tti ~ excess_water_scaled + lfm+ spp + site + year_month + (1 | individual), data = mem_data_all)
...
## Site, species, year & month, sample_wt
```{r}
mpa_mod_fig.mpa <- lmer(tti ~ mpa + spp + site + year + month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(mpa_mod_fig.mpa)
```

```{r}
max_model <- lmer(tti ~ spp + dw_flam_sample + ww_flam_sample + mpa + lfm+ excess_water_scaled + site + year + month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(max_model)
```

```{r}
max_model_nolfm <- lmer(tti ~ spp + dw_flam_sample + ww_flam_sample + mpa + excess_water_scaled + site +  year + month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(max_model_nolfm)
```

```{r}
max_model_nompa <- lmer(tti ~ spp + dw_flam_sample + ww_flam_sample + lfm+ excess_water_scaled + site +  year + month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(max_model_nompa)
```

```{r}
m3 <- lmer(tti ~ spp + ww_flam_sample + site + year + month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m3)
```

```{r}
m6 <- lmer(tti ~ dw_flam_sample + spp + site + year + month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m6)
```

```{r}
m6.5 <- lmer(tti ~ dw_flam_sample + ww_flam_sample + spp + site + year + month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m6.5)
```

```{r}
m7 <- lmer(tti ~ mpa + lfm+ spp + site + year + month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m7)
```

```{r}
m10 <- lmer(tti ~ lfm+ spp + site + year + month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m10)
```

```{r}
m11 <- lmer(tti ~ mpa*spp + site + year + month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m11)
```

```{r}
m12 <- lmer(tti ~ excess_water_scaled + spp + site + year + month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m12)
```

```{r}
m12.5 <- lmer(tti ~ excess_water_scaled + mpa + lfm+ spp + site + year + month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m12.5) ##High correlation! Don't use
```

```{r}
m13 <- lmer(tti ~ excess_water_scaled + lfm+ spp + site + year + month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m13) ##High correlation! Don't use
```

```{r}
m14 <- lmer(tti ~ excess_water_scaled + spp + site + year + month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m14)
```

```{r}
m15 <- lmer(tti ~ excess_water_scaled * spp + site + year + month + sample_wt + (1 | individual), data = mem_data_all)
...
tab_model(mpa_mod_fig.mpa, m11, m7, m10, m3, m6, m6.5, m12, m15,
  ...
  string.p = "P-Value", 
  p.style = "stars",
  file = here('figures', 'MEM_figures','site_yearANDmonth_samplewt.html')) # saving _html into figures folder
```

## Site, species, year & month
```{r}
mpa_mod_fig.mpa <- lmer(tti ~ mpa + spp + site + year + month + (1 | individual), data = mem_data_all)

performance::multicollinearity(mpa_mod_fig.mpa)
```

```{r}
max_model <- lmer(tti ~ spp + dw_flam_sample + ww_flam_sample + mpa + lfm+ excess_water_scaled + site + year + month + (1 | individual), data = mem_data_all)

performance::multicollinearity(max_model)
```

```{r}
max_model_nolfm <- lmer(tti ~ spp + dw_flam_sample + ww_flam_sample + mpa + excess_water_scaled + site +  year + month + (1 | individual), data = mem_data_all)

performance::multicollinearity(max_model_nolfm)
```

```{r}
max_model_nompa <- lmer(tti ~ spp + dw_flam_sample + ww_flam_sample + lfm+ excess_water_scaled + site +  year + month + (1 | individual), data = mem_data_all)

performance::multicollinearity(max_model_nompa)
```

```{r}
m3 <- lmer(tti ~ spp + ww_flam_sample + site + year + month + (1 | individual), data = mem_data_all)

performance::multicollinearity(m3)
```

```{r}
m6 <- lmer(tti ~ dw_flam_sample + spp + site + year + month + (1 | individual), data = mem_data_all)

performance::multicollinearity(m6)
```

```{r}
m6.5 <- lmer(tti ~ dw_flam_sample + ww_flam_sample + spp + site + year + month + (1 | individual), data = mem_data_all)

performance::multicollinearity(m6.5)
```

```{r}
m7 <- lmer(tti ~ mpa + lfm+ spp + site + year + month + (1 | individual), data = mem_data_all)

performance::multicollinearity(m7)
```

```{r}
m10 <- lmer(tti ~ lfm+ spp + site + year + month + (1 | individual), data = mem_data_all)

performance::multicollinearity(m10)
```

```{r}
m11 <- lmer(tti ~ mpa*spp + site + year + month + (1 | individual), data = mem_data_all)

performance::multicollinearity(m11)
```

```{r}
m12 <- lmer(tti ~ excess_water_scaled + spp + site + year + month + (1 | individual), data = mem_data_all)

performance::multicollinearity(m12)
```

```{r}
m12.5 <- lmer(tti ~ excess_water_scaled + mpa + lfm+ spp + site + year + month + (1 | individual), data = mem_data_all)

performance::multicollinearity(m12.5) ##High correlation! Don't use
```

```{r}
m13 <- lmer(tti ~ excess_water_scaled + lfm+ spp + site + year + month + (1 | individual), data = mem_data_all)

performance::multicollinearity(m13) ##High correlation! Don't use
```

```{r}
m14 <- lmer(tti ~ excess_water_scaled + spp + site + year + month + (1 | individual), data = mem_data_all)

performance::multicollinearity(m14)
```

```{r}
m15 <- lmer(tti ~ excess_water_scaled * spp + site + year + month + (1 | individual), data = mem_data_all)
...
tab_model(mpa_mod_fig.mpa, m11, m7, m10, m3, m6, m6.5, m12, m15,
  ...
  string.p = "P-Value", 
  p.style = "stars",
  file = here('figures', 'MEM_figures','site_yearANDmonth.html')) # saving _html into figures folder
```
# ---------------------------------------
# MAIN MODEL SELECTION
From our first round of model selection, it appears that site, species, and year_month are all important variables to include, and sample_wt should probably be included as well despite worsening the models slightly.
...
```{r}
m1 <- lmer(tti ~ lfm + spp + site + year_month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m1)
```

```{r}
m2 <- lmer(tti ~ mpa + spp + site + year_month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m2)
```

```{r}
m3 <- lmer(tti ~ dw_flam_sample + spp + site + year_month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m3)
```

```{r}
m4 <- lmer(tti ~ ww_flam_sample + spp + site + year_month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m4)
```

```{r}
m5 <- lmer(tti ~ excess_water_scaled + spp + site + year_month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m5)
```

```{r}
m6 <- lmer(tti ~ lfm + mpa + spp + site + year_month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m6)
```

```{r}
m7 <- lmer(tti ~ mpa + dw_flam_sample + spp + site + year_month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m7)
```

```{r}
m8 <- lmer(tti ~ mpa + ww_flam_sample + spp + site + year_month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m8)
```

```{r}
m9 <- lmer(tti ~ mpa + excess_water_scaled + spp + site + year_month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m9)
```

```{r}
m10 <- lmer(tti ~ mpa + dw_flam_sample + ww_flam_sample + spp + site + year_month + (1 | individual), data = mem_data_all)

performance::multicollinearity(m10)
```

```{r}
m11 <- lmer(tti ~ dw_flam_sample + excess_water_scaled + spp + site + year_month + (1 | individual), data = mem_data_all)

performance::multicollinearity(m11)
```

```{r}
m12 <- lmer(tti ~ dw_flam_sample + ww_flam_sample + spp + site + year_month + (1 | individual), data = mem_data_all)
...
tab_model(m1, m2, m3, m4, m5, m6, m7, m8, m9, m10, m11, m12,
  ...
  string.p = "P-Value", 
  p.style = "stars",
  file = here('figures', 'MEM_figures','modelselection.html')) # saving .html into figures folder
```
...
## tti
```{r}
c.p1 <- lmer(tti ~ mpa + site + year_month + (1 | individual), data = mem.CEME.subset)

c.p2 <- lmer(tti ~ lfm + site + year_month + (1 | individual), data = mem.CEME.subset)

c.p3 <- lmer(tti ~ ww_flam_sample + site + year_month + (1 | individual), data = mem.CEME.subset)

c.p4 <- lmer(tti ~ dw_flam_sample + site + year_month + (1 | individual), data = mem.CEME.subset)

c.p5 <- lmer(tti ~ mpa + dw_flam_sample + sample_wt   + site + year_month + (1 | individual), data = mem.CEME.subset)
```

```{r}
a.p1 <- lmer(tti ~ mpa + site + year_month + (1 | individual), data = mem.adfa.subset)

a.p2 <- lmer(tti ~ lfm + site + year_month + (1 | individual), data = mem.adfa.subset)

a.p3 <- lmer(tti ~ ww_flam_sample + site + year_month + (1 | individual), data = mem.adfa.subset)

a.p4 <- lmer(tti ~ dw_flam_sample + site + year_month + (1 | individual), data = mem.adfa.subset)

a.p5 <- lmer(tti ~ mpa + dw_flam_sample + sample_wt + site + year_month + (1 | individual), data = mem.adfa.subset)
```
...
tab_model(a.p1, a.p5, a.p3, a.p2, a.p4, c.p1, c.p3, c.p4, c.p5, c.p2,
  ...
  string.p = "P-Value", 
  p.style = "stars",
  file = here('figures', 'MEM_figures','tti_by_species.html')) # saving .html into figures folder
```

## Time to Ignition
```{r}
c.t1 <- lmer(tti ~ mpa + site + year_month + (1 | individual), data = mem.CEME.subset)

c.t2 <- lmer(tti ~ lfm + site + year_month + (1 | individual), data = mem.CEME.subset)

c.t3 <- lmer(tti ~ ww_flam_sample + site + year_month + (1 | individual), data = mem.CEME.subset)

c.t4 <- lmer(tti ~ dw_flam_sample + site + year_month + (1 | individual), data = mem.CEME.subset)

c.t5 <- lmer(tti ~ mpa + dw_flam_sample + sample_wt   + site + year_month + (1 | individual), data = mem.CEME.subset)
```

```{r}
a.t1 <- lmer(tti ~ mpa + site + year_month + (1 | individual), data = mem.adfa.subset)

a.t2 <- lmer(tti ~ lfm + site + year_month + (1 | individual), data = mem.adfa.subset)

a.t3 <- lmer(tti ~ ww_flam_sample + site + year_month + (1 | individual), data = mem.adfa.subset)

a.t4 <- lmer(tti ~ dw_flam_sample + site + year_month + (1 | individual), data = mem.adfa.subset)

a.t5 <- lmer(tti ~ mpa + dw_flam_sample + sample_wt + site + year_month + (1 | individual), data = mem.adfa.subset)
```
...
## Flame Duration
```{r}
c.f1 <- lmer(fd ~ mpa + site + year_month + (1 | individual), data = mem.CEME.subset)

c.f2 <- lmer(fd ~ lfm + site + year_month + (1 | individual), data = mem.CEME.subset)

c.f3 <- lmer(fd ~ ww_flam_sample + site + year_month + (1 | individual), data = mem.CEME.subset)

c.f4 <- lmer(fd ~ dw_flam_sample + site + year_month + (1 | individual), data = mem.CEME.subset)

c.f5 <- lmer(fd ~ mpa + dw_flam_sample + sample_wt   + site + year_month + (1 | individual), data = mem.CEME.subset)
```

```{r}
a.f1 <- lmer(fd ~ mpa + site + year_month + (1 | individual), data = mem.adfa.subset)

a.f2 <- lmer(fd ~ lfm + site + year_month + (1 | individual), data = mem.adfa.subset)

a.f3 <- lmer(fd ~ ww_flam_sample + site + year_month + (1 | individual), data = mem.adfa.subset)

a.f4 <- lmer(fd ~ dw_flam_sample + site + year_month + (1 | individual), data = mem.adfa.subset)

a.f5 <- lmer(fd ~ mpa + dw_flam_sample + sample_wt + site + year_month + (1 | individual), data = mem.adfa.subset)
```
...
tab_model(a.f1, a.f4, a.f2, a.f3, a.f5, c.f4, c.f2, c.f1, c.f3, c.f5,
  ...
  string.p = "P-Value", 
  p.style = "stars",
  file = here('figures', 'MEM_figures','FD_by_species.html')) # saving .html into figures folder
```

## Glow Duration
```{r}
c.g1 <- lmer(gd ~ mpa + site + year_month + (1 | individual), data = mem.CEME.subset)

c.g2 <- lmer(gd ~ lfm + site + year_month + (1 | individual), data = mem.CEME.subset)

c.g3 <- lmer(gd ~ ww_flam_sample + site + year_month + (1 | individual), data = mem.CEME.subset)

c.g4 <- lmer(gd ~ dw_flam_sample + site + year_month + (1 | individual), data = mem.CEME.subset)

c.g5 <- lmer(gd ~ mpa + dw_flam_sample + sample_wt   + site + year_month + (1 | individual), data = mem.CEME.subset)
```

```{r}
a.g1 <- lmer(gd ~ mpa + site + year_month + (1 | individual), data = mem.adfa.subset)

a.g2 <- lmer(gd ~ lfm + site + year_month + (1 | individual), data = mem.adfa.subset)

a.g3 <- lmer(gd ~ ww_flam_sample + site + year_month + (1 | individual), data = mem.adfa.subset)

a.g4 <- lmer(gd ~ dw_flam_sample + site + year_month + (1 | individual), data = mem.adfa.subset)

a.g5 <- lmer(gd ~ mpa + dw_flam_sample + sample_wt + site + year_month + (1 | individual), data = mem.adfa.subset)
```
...
tab_model(a.g5, a.g4, a.g2, a.g3, a.g1, c.g5, c.g4, c.g1, c.g2, c.g3,
  ...
  string.p = "P-Value", 
  p.style = "stars",
  file = here('figures', 'MEM_figures','GD_by_species.html')) # saving _html into figures folder
```
...
```{r}
m1 <- lmer(PC2 ~ lfm + spp + site + year_month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m1)
```

```{r}
m2 <- lmer(PC2 ~ mpa + spp + site + year_month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m2)
```

```{r}
m3 <- lmer(PC2 ~ dw_flam_sample + spp + site + year_month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m3)
```

```{r}
m4 <- lmer(PC2 ~ ww_flam_sample + spp + site + year_month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m4)
```

```{r}
m5 <- lmer(PC2 ~ excess_water_scaled + spp + site + year_month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m5)
```

```{r}
m6 <- lmer(PC2 ~ lfm + mpa + spp + site + year_month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m6)
```

```{r}
m7 <- lmer(PC2 ~ mpa + dw_flam_sample + spp + site + year_month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m7)
```

```{r}
m8 <- lmer(PC2 ~ mpa + ww_flam_sample + spp + site + year_month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m8)
```

```{r}
m9 <- lmer(PC2 ~ mpa + excess_water_scaled + spp + site + year_month + sample_wt + (1 | individual), data = mem_data_all)

performance::multicollinearity(m9)
```

```{r}
m10 <- lmer(PC2 ~ mpa + dw_flam_sample + ww_flam_sample + spp + site + year_month + (1 | individual), data = mem_data_all)

performance::multicollinearity(m10)
```

```{r}
m11 <- lmer(PC2 ~ dw_flam_sample + excess_water_scaled + spp + site + year_month + (1 | individual), data = mem_data_all)

performance::multicollinearity(m11)
```

```{r}
m12 <- lmer(PC2 ~ dw_flam_sample + ww_flam_sample + spp + site + year_month + (1 | individual), data = mem_data_all)
...
### Models
```{r}
m.p1 <- lmer(tti ~ mpa + spp + site + year_month + sample_wt + (1 | individual), data = mem_data_all)
  
m.p2 <- lmer(PC2 ~ mpa + spp + site + year_month + sample_wt + (1 | individual), data = mem_data_all)
```
...
p3 <- ggplot(r_mem_data_all, aes(x = dw_flam_sample, y = r_pc2)) +
  geom_point(alpha = 0.6, size = 1) +
...
p4 <- ggplot(r_mem_data_all, aes(x = dw_flam_sample, y = PC2)) +
  geom_point(alpha = 0.6, size = 1) +
  labs(x = "Dry Weight (scaled, centered)") +
  geom_smooth(method = 'lm', color = 'black') +
  facet_wrap(~spp) + 
...
model <- lmer(tti ~ mpa + dw_flam_sample + spp.n + site.n + year_month + sample_wt + (1 | individual), data = comp.model.df)
...
p <- ggplot() + 
  geom_point(data= mem_data_all, aes(mpa, tti, color = Species), size = 1) + 
...
Models
```{r}
m1 <- lmer(tti ~ mpa + spp + site + year_month + (1 | individual), data = mem_data_all)

m2 <- lmer(tti ~ lfm + spp + site + year_month + (1 | individual), data = mem_data_all)

m3 <- lmer(tti ~ ww_flam_sample + spp + site + year_month + (1 | individual), data = mem_data_all)

m4 <- lmer(tti ~ dw_flam_sample + spp + site + year_month + (1 | individual), data = mem_data_all)

m5 <- lmer(tti ~ mpa + dw_flam_sample + sample_wt + spp + site + year_month + (1 | individual), data = mem_data_all)
```
...
Models
```{r}
m1 <- lmer(fh ~ mpa + spp + site + year_month + (1 | individual), data = mem_data_all)

m2 <- lmer(fh ~ lfm + spp + site + year_month + (1 | individual), data = mem_data_all)

m3 <- lmer(fh ~ ww_flam_sample + spp + site + year_month + (1 | individual), data = mem_data_all)

m4 <- lmer(fh ~ dw_flam_sample + spp + site + year_month + (1 | individual), data = mem_data_all)

m5 <- lmer(fh ~ mpa + dw_flam_sample + sample_wt + spp + site + year_month + (1 | individual), data = mem_data_all)
```
...
Models
```{r}
m1 <- lmer(gd ~ mpa + spp + site + year_month + (1 | individual), data = mem_data_all)

m2 <- lmer(gd ~ lfm + spp + site + year_month + (1 | individual), data = mem_data_all)

m3 <- lmer(gd ~ ww_flam_sample + spp + site + year_month + (1 | individual), data = mem_data_all)

m4 <- lmer(gd ~ dw_flam_sample + spp + site + year_month + (1 | individual), data = mem_data_all)

m5 <- lmer(gd ~ mpa + dw_flam_sample + sample_wt + spp + site + year_month + (1 | individual), data = mem_data_all)
```
...
Models
```{r}
m1 <- lmer(fd ~ mpa + spp + site + year_month + (1 | individual), data = mem_data_all)

m2 <- lmer(fd ~ lfm + spp + site + year_month + (1 | individual), data = mem_data_all)

m3 <- lmer(fd ~ ww_flam_sample + spp + site + year_month + (1 | individual), data = mem_data_all)

m4 <- lmer(fd ~ dw_flam_sample + spp + site + year_month + (1 | individual), data = mem_data_all)

m5 <- lmer(fd ~ mpa + dw_flam_sample + sample_wt + spp + site + year_month + (1 | individual), data = mem_data_all)
```
...
Models
```{r}
m1 <- lmer(temp.max ~ mpa + spp + site + year_month + (1 | individual), data = mem_data_all)

m2 <- lmer(temp.max ~ lfm + spp + site + year_month + (1 | individual), data = mem_data_all)

m3 <- lmer(temp.max ~ ww_flam_sample + spp + site + year_month + (1 | individual), data = mem_data_all)

m4 <- lmer(temp.max ~ dw_flam_sample + spp + site + year_month + (1 | individual), data = mem_data_all)

m5 <- lmer(temp.max ~ mpa + dw_flam_sample + sample_wt + spp + site + year_month + (1 | individual), data = mem_data_all)
```
...
Models
```{r}
m1 <- lmer(pfg ~ mpa + spp + site + year_month + (1 | individual), data = mem_data_all)

m2 <- lmer(pfg ~ lfm + spp + site + year_month + (1 | individual), data = mem_data_all)

m3 <- lmer(pfg ~ ww_flam_sample + spp + site + year_month + (1 | individual), data = mem_data_all)

m4 <- lmer(pfg ~ dw_flam_sample + spp + site + year_month + (1 | individual), data = mem_data_all)

m5 <- lmer(pfg ~ mpa + dw_flam_sample + sample_wt + spp + site + year_month + (1 | individual), data = mem_data_all)
```
...
Models
```{r}
m1 <- lmer(ttfg ~ mpa + spp + site + year_month + (1 | individual), data = mem_data_all)

m2 <- lmer(ttfg ~ lfm + spp + site + year_month + (1 | individual), data = mem_data_all)

m3 <- lmer(ttfg ~ ww_flam_sample + spp + site + year_month + (1 | individual), data = mem_data_all)

m4 <- lmer(ttfg ~ dw_flam_sample + spp + site + year_month + (1 | individual), data = mem_data_all)

m5 <- lmer(ttfg ~ mpa + dw_flam_sample + sample_wt + spp + site + year_month + (1 | individual), data = mem_data_all)
```
...
Models
```{r}
m1 <- lmer(gti ~ mpa + spp + site + year_month + (1 | individual), data = mem_data_all)

m2 <- lmer(gti ~ lfm + spp + site + year_month + (1 | individual), data = mem_data_all)

m3 <- lmer(gti ~ ww_flam_sample + spp + site + year_month + (1 | individual), data = mem_data_all)

m4 <- lmer(gti ~ dw_flam_sample + spp + site + year_month + (1 | individual), data = mem_data_all)

m5 <- lmer(gti ~ mpa + dw_flam_sample + sample_wt + spp + site + year_month + (1 | individual), data = mem_data_all)
```
...
```{r}
m3 <- lm(tti ~ mpa + sample_wt + dw_flam_sample, data = mem_data_all)
res.m3 <- resid(m3)
...
```{r}
m4 <- lm(tti ~ mpa + site + spp + year_month, data = mem_data_all)
res.m4 <- resid(m4)
...
mem_data_all <- mem_data_all %>% 
  mutate(excess.dw.centered = dw_flam_sample.unscaled - mean(dw_flam_sample.unscaled)) %>% 
  mutate(excess.dw.percent = (dw_flam_sample.unscaled - mean(dw_flam_sample.unscaled))/mean(dw_flam_sample.unscaled))
```

```{r}
m1 <- lmer(tti ~ spp + excess.dw.centered + excess_water.unscaled + sample_wt + site + year_month + (1 | individual), data = mem_data_all)
...
```{r}
m2 <- lmer(tti ~ spp + excess.dw.centered + excess_water.unscaled + site + year_month + (1 | individual), data = mem_data_all)
...
```{r}
m3 <- lmer(tti ~ spp + excess.dw.percent + excess_water.unscaled + sample_wt + site + year_month + (1 | individual), data = mem_data_all)
...
```{r}
m4 <- lmer(tti ~ spp + excess.dw.percent + excess_water.unscaled + site + year_month + (1 | individual), data = mem_data_all)
...
```{r}
m5 <- lmer(tti ~ spp + excess.dw.centered + mpa + sample_wt + site + year_month + (1 | individual), data = mem_data_all)
...
```{r}
m6 <- lmer(tti ~ spp + excess.dw.centered + lfm + sample_wt + site + year_month + (1 | individual), data = mem_data_all)
...
```{r}
m7 <- lmer(tti ~ spp + dw_flam_sample + excess_water_scaled + sample_wt + site + year_month + (1 | individual), data = mem_data_all)
...
```{r}
m8 <- lmer(tti ~ spp + dw_flam_sample + mpa + sample_wt + site + year_month + (1 | individual), data = mem_data_all)
