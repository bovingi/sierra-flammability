...
# Data Wrangling
For the segmented regressions, we debated using seasonal mpa minimums and maximums to limit the dataset, but instead -- since we wanted to keep the datasets similar for each analysis -- we used the dataset that was used in the mixed effects model analysis

## Species Split
```{r}
seg_pije_subset <- seg_data_subset %>% 
  filter(spp == "pije") %>% 
  group_by(mpa_bin = cut(lfm, breaks= seq(0, 200, by = 6))) %>%  #Fixing the bins, setting based on pije max
  drop_na(mpa, year_month, site, id) %>% 
  select(tti, fh, temp_change, gd, gti, pfg, ttfg, fd, prop_ignite, mpa, mpa_bin, mpa, year_month, site, id)
```
...
median(pije_bin_n$n) # Median: 7.5, round to 8
```
...
pije_binned_df <- seg_pije_subset %>% 
  group_by(mpa_bin) %>% 
  sample_n(8, replace = T) #from above
```

# 3. Segmented Regression
```{r}
out_lm<-lm(tti ~ mpa + site + year_month, data = pije_binned_df)
davies.test(out_lm, ~mpa, k = 50)

segmented_mod <- segmented(out_lm, seg.Z = ~mpa) #1 breakpoint for x
...
for(i in 1:10){
  ...
  # if(i == 1){
  #   out_lm<-lm(PC1 ~ mpa + site + year_month, data = pije_binned_df)
  # segmented_mod <- segmented(out_lm, seg.Z = ~mpa)
  # pije_thresholds[j,i] <- summary(segmented_mod)$psi[2]
  # }
  # else if(i == 2) {
  # out_lm<-lm(PC2 ~ mpa + site + year_month, data = pije_binned_df)
  # segmented_mod <- segmented(out_lm, seg.Z = ~mpa)
  # pije_thresholds[j,i] <- summary(segmented_mod)$psi[2]
  # }
  if(i == 1) {
  ...
  pije_thresholds[j,i] <- summary(segmented_mod)$psi[2]
    ...
    out_lm<-lm(fh ~ mpa + site + year_month, data = pije_binned_df)
  segmented_mod <- segmented(out_lm, seg.Z = ~mpa)
  pije_thresholds[j,i] <- summary(segmented_mod)$psi[2]
  }
  else if(i == 3) {
    out_lm<-lm(fd ~ mpa + site + year_month, data = pije_binned_df)
  segmented_mod <- segmented(out_lm, seg.Z = ~mpa)
  pije_thresholds[j,i] <- summary(segmented_mod)$psi[2]
  }
  else if(i == 4) {
    out_lm<-lm(gd ~ mpa + site + year_month, data = pije_binned_df)
  segmented_mod <- segmented(out_lm, seg.Z = ~mpa)
  pije_thresholds[j,i] <- summary(segmented_mod)$psi[2]
  }
  else if(i == 5) {
    out_lm<-lm(gti ~ mpa + site + year_month, data = pije_binned_df)
  segmented_mod <- segmented(out_lm, seg.Z = ~mpa)
  pije_thresholds[j,i] <- summary(segmented_mod)$psi[2]
  }
  else if(i == 6) {
    out_lm<-lm(pfg ~ mpa + site + year_month, data = pije_binned_df)
  segmented_mod <- segmented(out_lm, seg.Z = ~mpa)
  pije_thresholds[j,i] <- summary(segmented_mod)$psi[2]
  }
  else if(i == 7) {
    out_lm<-lm(ttfg ~ mpa + site + year_month, data = pije_binned_df)
  segmented_mod <- segmented(out_lm, seg.Z = ~mpa)
  pije_thresholds[j,i] <- summary(segmented_mod)$psi[2]
  }
    else if(i == 8) {
    out_lm<-lm(prop_ignite ~ mpa + site + year_month, data = pije_binned_df)
  segmented_mod <- segmented(out_lm, seg.Z = ~mpa)
  pije_thresholds[j,i] <- summary(segmented_mod)$psi[2]
    }
    else if(i == 9) {
    out_lm<-lm(temp_change ~ mpa + site + year_month, data = pije_binned_df)
  segmented_mod <- segmented(out_lm, seg.Z = ~mpa)
  pije_thresholds[j,i] <- summary(segmented_mod)$psi[2]
  }
   }}
...
It all worked! Can comment out the code below. 

JOE's comments: 
Since the above for loop kept getting interrupted when thresholds were not discovered, I set up a new for loop to figure out which seeds are causing the interruptions and manually removing those seeds. For pije, I did this for glow duration, glow to ignition, proportion ignited, and temp change (in Joes code prev max. temp). 
...
```{r}
pije_summary_df <- pije_thresholds %>% 
  pivot_longer(everything(), 
               names_to = "flam_metric", 
              values_to = "value") %>% 
  group_by(flam_metric) %>% 
  summarise(mean_bootstrap = mean(value), 
...
#   save_kable(here('figures', 'supp-figures', 'mpa_segreg_summary_html'))
```
