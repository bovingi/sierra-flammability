...
## Species Split
```{r}
seg_abco_subset <- seg_data_subset %>% 
  filter(spp == "abco") %>% 
  group_by(lfm_bin = cut(lfm, breaks= seq(0, 236, by = 6))) %>%  #Fixing the bins, setting based on abco max
...
```{r}
abco_bin_n <- seg_abco_subset %>% 
  group_by(lfm_bin) %>% 
  count()

median(abco_bin_n$n) # Median: 9
```

# 2. Resampling points from each bin
```{r}
abco_binned_df <- seg_abco_subset %>% 
  group_by(lfm_bin) %>% 
...
```{r}
out_lm<-lm(tti ~ lfm + site + year_month, data = abco_binned_df)
...
abco_thresholds <- data_frame(psi_tti, psi_fh, psi_fd, psi_gd, psi_gti, psi_pfg, psi_ttfg, psi_prop_ignite, psi_temp_change)
```
...
```{r}
for(i in 1:10){
   for(j in 1:nrow(abco_thresholds)){
  abco_binned_df <- seg_abco_subset %>% 
  group_by(lfm_bin) %>% 
  sample_n(19, replace = T)
  # if(i == 1){
  #   out_lm<-lm(PC1 ~ lfm + site + year_month, data = abco_binned_df)
  # segmented_mod <- segmented(out_lm, seg.Z = ~lfm)
  # abco_thresholds[j,i] <- summary(segmented_mod)$psi[2]
  # }
  # else if(i == 2) {
  # out_lm<-lm(PC2 ~ lfm + site + year_month, data = abco_binned_df)
  # segmented_mod <- segmented(out_lm, seg.Z = ~lfm)
  # abco_thresholds[j,i] <- summary(segmented_mod)$psi[2]
  # }
  if(i == 1) {
  ...
  abco_thresholds[j,i] <- summary(segmented_mod)$psi[2]
    ...
    out_lm<-lm(fh ~ lfm + site + year_month, data = abco_binned_df)
  segmented_mod <- segmented(out_lm, seg.Z = ~lfm)
  abco_thresholds[j,i] <- summary(segmented_mod)$psi[2]
  }
  # else if(i == 3) {
  #   out_lm<-lm(fd ~ lfm + site + year_month, data = abco_binned_df)
  # segmented_mod <- segmented(out_lm, seg.Z = ~lfm)
  # abco_thresholds[j,i] <- summary(segmented_mod)$psi[2]
  # }
  else if(i == 4) {
    out_lm<-lm(gd ~ lfm + site + year_month, data = abco_binned_df)
  segmented_mod <- segmented(out_lm, seg.Z = ~lfm)
  abco_thresholds[j,i] <- summary(segmented_mod)$psi[2]
  }
  else if(i == 5) {
    out_lm<-lm(gti ~ lfm + site + year_month, data = abco_binned_df)
  segmented_mod <- segmented(out_lm, seg.Z = ~lfm)
  abco_thresholds[j,i] <- summary(segmented_mod)$psi[2]
  }
  else if(i == 6) {
    out_lm<-lm(pfg ~ lfm + site + year_month, data = abco_binned_df)
  segmented_mod <- segmented(out_lm, seg.Z = ~lfm)
  abco_thresholds[j,i] <- summary(segmented_mod)$psi[2]
  }
  else if(i == 7) {
    out_lm<-lm(ttfg ~ lfm + site + year_month, data = abco_binned_df)
  segmented_mod <- segmented(out_lm, seg.Z = ~lfm)
  abco_thresholds[j,i] <- summary(segmented_mod)$psi[2]
  }
    else if(i == 8) {
    out_lm<-lm(prop_ignite ~ lfm + site + year_month, data = abco_binned_df)
  segmented_mod <- segmented(out_lm, seg.Z = ~lfm)
  abco_thresholds[j,i] <- summary(segmented_mod)$psi[2]
    }
    else if(i == 9) {
    out_lm<-lm(temp_change ~ lfm + site + year_month, data = abco_binned_df)
  segmented_mod <- segmented(out_lm, seg.Z = ~lfm)
  abco_thresholds[j,i] <- summary(segmented_mod)$psi[2]
  }
   }}
...
for(j in 1:67){
  ...
  abco_binned_df <- seg_abco_subset %>%
  group_by(lfm_bin) %>%
  sample_n(57, replace = T)

  out_lm<-lm(fh ~ lfm + site + year_month, data = abco_binned_df)
  segmented_mod <- segmented(out_lm, seg.Z = ~lfm)
  abco_thresholds$psi_fh[j] <- summary(segmented_mod)$psi[2]}
...
for(j in 1:100){
  ...
  out_lm<-lm(fd ~ lfm + site + year_month, data = abco_binned_df)
  segmented_mod <- segmented(out_lm, seg.Z = ~lfm)
  abco_thresholds$psi_fd[j] <- summary(segmented_mod)$psi[2]}
```
...
for(j in 1:100){
  ...
  out_lm<-lm(gd ~ lfm + site + year_month, data = abco_binned_df)
  segmented_mod <- segmented(out_lm, seg.Z = ~lfm)
  abco_thresholds$psi_gd[j] <- summary(segmented_mod)$psi[2]}
```
...
for(j in 1:100){
  ...
  out_lm<-lm(gti ~ lfm + site + year_month, data = abco_binned_df)
  segmented_mod <- segmented(out_lm, seg.Z = ~lfm)
  abco_thresholds$psi_gti[j] <- summary(segmented_mod)$psi[2]}
```

Skipping pfg and ttfg
...
for(j in 1:100){
  ...
  out_lm<-lm(prop_ignite ~ lfm + site + year_month, data = abco_binned_df)
  segmented_mod <- segmented(out_lm, seg.Z = ~lfm)
  abco_thresholds$psi_prop_ignite[j] <- summary(segmented_mod)$psi[2]}
```
...
for(j in 1:100){
  ...
  out_lm<-lm(temp_change ~ lfm + site + year_month, data = abco_binned_df)
  segmented_mod <- segmented(out_lm, seg.Z = ~lfm)
  abco_thresholds$psi_temp_change[j] <- summary(segmented_mod)$psi[2]}
```
...
```{r}
abco_summary_df <- abco_thresholds %>% 
...
# spp <- c(rep('abco', 11), 'TLP = 74_33 +/- 7_37', rep('CEME', 11), 'TLP = 88_96 +/- 2_53')
