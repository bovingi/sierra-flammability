...
tti_plot_lfm_r <- mem_data_all_r %>%
  ...
  geom_smooth(method = "lm", se = F, aes(color = F.Group, lty = F.tti.sig), alpha = .75, lty = 3) + 
...
summary(lower_temp_max_mod_fgroup)

r_lower_temp_max_mpa <- remef(lower_temp_max_mod, fix = c("sample_wt_scaled", "lfm_scaled", "site"), ran = list(individual = c("(Intercept)"))) # Using Remef for plots with MPa
r_lower_temp_max_lfm <- remef(lower_temp_max_mod, fix = c("mpa_scaled", "sample_wt_scaled", "site"), ran = list(individual = c("(Intercept)"))) # Using Remef for plots with LFM
...
lower_temp_max_plot_lfm_r <- mem_data_all_r %>% 
  ...
  geom_smooth(method = "lm", se = F, aes(color = F.Group,
                   lty = F.temp.max.lfm.sig), alpha = .75, lty = 1) +
  ...
  annotate(geom = "text", x = -2.6, y = 515, label = 'g', fontface = 'bold', size = 10) +
  annotate(geom = 'text', x = -2.3, y = 170, label = 'dry', size = 5) +
  annotate(geom = 'text', x = 2.3, y = 170, label = 'wet', size = 5) +
  geom_segment(aes(x = -2, y = 170, xend = 2, yend = 170),
...
boxplot.no.tlp.split <- df %>% filter(F.Group == "Angiosperm") %>% 
   ...
   annotate('text', x = 0.7, y = 115, label = 'j', fontface = 'bold', size = 12) +
...
tab_model(tti_mod_fgroup, fh_mod_fgroup, gd_mod_fgroup,temp_change_mod.fgroup,
        ...
        dv.labels = c("Time to Ignition", "Flame Height","Glow Duration", "Temp Change"),
...
No F group: 
```{r}
tab_model(tti_mod, fh_mod, gd_mod, prop_ignite_mod,
          show.reflvl = TRUE, 
          digits = 3, 
          show.aic = TRUE, 
          show.ci = FALSE,
...
sjPlot::plot_model(tti_spp)

fh_spp <- lmer(fh ~ spp + lfm_scaled + (1 | individual), data = mem_data_all_r)

sjPlot::plot_model(fh_spp)

tmp_spp <- lmer(temp_change ~ spp + lfm_scaled + (1 | individual), data = mem_data_all_r)

sjPlot::plot_model(tmp_spp)
```
...
tti.ES.lfm <- c(8.7, -1.6, 7.8, 9.7, -3.5) # Following this order: ABCO, CADE, PIJE, ARPA, QUKE (no CECO)
tti.SE.lfm <- c(3.372, 6.845, 5.582, 5.131, 7.569)
tti.sig.lfm <- c('Yes', 'No', 'No', 'Yes', 'No')
tti.ES.mpa <- c(-7.2,4.2, -4.6, 1.5, 1.4) 
tti.SE.mpa <- c(5.496, 7.903, 7.483, 6.536, 7.443)
...
tti.ES.lfm.plot <- ggplot(data = tti.ES.df, aes(x = species, y = tti.ES.lfm, 
 ...
 # geom_vline(xintercept = 3.4, alpha = 0.5, size = 0.3, lty = 2) +
  theme_bw() +
  scale_y_continuous(limits = c(-12.5, 14.5), breaks = c(-10,-5, 0, 5, 10)) +
  color_noceco+
  #scale_color_manual(values = c('black', '#FDD358', 'black', 'black', '#FDD358')) + # For color by angio vs gymno (version 1)
  scale_shape_manual(values = c(10, 16)) +
  annotate(geom = "text", x = 0.9, y = 12, label = 'b', fontface = 'bold', size = 10) +
...
tti.ES.mpa.plot <- ggplot(data = tti.ES.df, aes(x = species, y = tti.ES.mpa,
 ...
 # geom_vline(xintercept = 3.4, alpha = 0.5, size = 0.3, lty = 2) +
  ...
  #scale_color_manual(values = c('black', '#FDD358', 'black', 'black', '#FDD358')) + # For color by angio vs gymno (version 1)
  scale_shape_manual(values = c(10, 16)) +
  annotate(geom = "text", x = 0.9, y = 12, label = 'c', fontface = 'bold', size = 10) +
 # annotate(geom = "text", x = 2, y = 14, label = 'Gymnosperms', size = 4) +
 # annotate(geom = "text", x = 4.52, y = 14, label = 'Angiosperms', size = 4) +
  theme(axis.title.x = element_blank(), axis.text.x = element_blank(),
...
fh.ES.lfm <- c(-2, -7.3, -3.6, -2.4, -7.2) # Following this order: ABCO, CADE, PIJE, ARPA, QUKE
fh.SE.lfm <- c(0.96460, 1.95516, 1.59944, 1.47320, 2.16010)
fh.sig.lfm <- c('Yes', 'No', 'No', 'Yes', 'No')
fh.ES.mpa <- c(-4.5, -0.2, -1.9, -0.73, -0.41) 
fh.SE.mpa <- c(1.56526, 2.25387, 2.13386, 1.86357, 2.12124)
fh.sig.mpa <- c('Yes', 'No', 'No', 'Yes', 'Yes')
...
fh.ES.lfm.plot <- ggplot(data = fh.ES.df, aes(x = species, y = fh.ES.lfm, 
 ...
 # geom_vline(xintercept = 3.4, alpha = 0.5, size = 0.3, lty = 2) +
  theme_bw() +
  scale_y_continuous(limits = c(-8.5, 3), breaks = c(-7.5,-5,-2.5, 0, 2.5)) +
  #color_many2 +
  color_noceco+
  #scale_color_manual(values = c('black', '#FDD358', 'black', 'black', '#FDD358')) + # For color by angio vs gymno (version 1)
  scale_shape_manual(values = c(10, 16)) +
  annotate(geom = "text", x = 0.9, y = 2.35, label = 'e', fontface = 'bold', size = 10) +
  theme(axis.title.x = element_blank(), axis.text.x = element_blank(),
        axis.title.y = element_blank(), 
        axis.text.y = element_blank(),
        axis.ticks.y = element_blank(),
...
fh.ES.mpa.plot <- ggplot(data = fh.ES.df, aes(x = species, y = fh.ES.mpa,
 ...
 # geom_vline(xintercept = 3.4, alpha = 0.5, size = 0.3, lty = 2) +
  ...
  #scale_color_manual(values = c('black', '#FDD358', 'black', 'black', '#FDD358')) + # For color by angio vs gymno (version 1)
  scale_shape_manual(values = c(10, 16)) +
  annotate(geom = "text", x = 0.9, y = 2.35, label = 'f', fontface = 'bold', size = 10) +
  theme(axis.title.x = element_blank(), axis.text.x = element_blank(),
        axis.title.y.right = element_text(face = 'bold', size = 14),
        axis.ticks.y.left = element_blank(), 
        axis.text.y.left = element_blank(),
...
## Max. Temp.
```{r}
summary(lower_temp_max_mod)

temp_max.ES.lfm <- c(0.38, 4.4, 14, 8.7, -17) # Following this order: ABCO, CADE, PIJE, ARPA, QUKE
temp_max.SE.lfm <- c(6.4496, 13.2046, 10.4746, 9.5420, 14.7575)
temp_max.sig.lfm <- c('No', 'No', 'No', 'No', 'No')
temp_max.ES.mpa <- c(3.1, 1.3, -16, -17, 16)
temp_max.SE.mpa <- c(10.9458, 15.5152, 14.7171, 12.9146, 14.7125)
temp_max.sig.mpa <- c('No', 'No', 'No', 'No', 'No')
temp_max.ES.df <- data.frame(species, temp_max.ES.lfm, temp_max.SE.lfm, temp_max.sig.lfm, temp_max.ES.mpa, temp_max.SE.mpa, temp_max.sig.mpa)

temp_max.ES.df$species <- factor(temp_max.ES.df$species, levels = c("A. concolor", "C. decurrens", "P. jeffreyi", "A. patula", "Q. kelloggii"))

temp_max.ES.lfm.plot <- ggplot(data = temp_max.ES.df, aes(x = species, y = temp_max.ES.lfm, 
                                                color = species, shape = temp_max.sig.lfm)) +
  labs(color = 'Species', shape = 'Significant?') +
  geom_point(size = 5) +
  geom_linerange(aes(ymin = temp_max.ES.lfm - 0.5*temp_max.SE.lfm, ymax = temp_max.ES.lfm + 0.5*temp_max.SE.lfm)) +
  geom_abline(intercept = 0, slope = 0, color = 'black', alpha = 0.5, size = 0.3) +
  #geom_vline(xintercept = 3.4, alpha = 0.5, size = 0.3, lty = 2) +
  theme_bw() +
  ...
  #scale_color_manual(values = c('black', '#FDD358', 'black', 'black', '#FDD358')) + # For color by angio vs gymno (version 1)
  scale_shape_manual(values = c(10, 16)) +
  annotate(geom = "text", x = 0.9, y = 25, label = 'h', fontface = 'bold', size = 10) +
  theme(axis.title.x = element_blank(), axis.text.x = element_blank(),
        axis.title.y = element_blank(), 
        axis.text.y = element_blank(),
        axis.ticks.y = element_blank(),
...
temp_max.ES.mpa.plot <- ggplot(data = temp_max.ES.df, aes(x = species, y = temp_max.ES.mpa,
                                                color = species, shape = temp_max.sig.mpa)) +
  labs(y = '', color = 'Species', shape = 'Significant?') +
  scale_y_continuous(limits = c(-30, 30), breaks = c(-20,-10, 0, 10, 20),
                     sec.axis = sec_axis(trans = ~.*1, 
                     #name = 'Hydration x Species Effect Size',
  ...
  geom_point(size = 5, cex = 5) +
  geom_linerange(aes(ymin = temp_max.ES.mpa - 0.5*temp_max.SE.mpa, ymax = temp_max.ES.mpa + 0.5*temp_max.SE.mpa)) +
  geom_abline(intercept = 0, slope = 0, color = 'black', alpha = 0.5, size = 0.3) +
  #geom_vline(xintercept = 3.4, alpha = 0.5, size = 0.3, lty = 2) +
  theme_bw() +
  #color_many2 +
  ...
  #scale_color_manual(values = c('black', '#FDD358', 'black', 'black', '#FDD358')) + # For color by angio vs gymno (version 1)
  scale_shape_manual(values = c(10, 16)) +
  annotate(geom = "text", x = 0.9, y = 25, label = 'i', fontface = 'bold', size = 10) +
  theme(axis.title.x = element_blank(), axis.text.x = element_blank(),
        axis.ticks.y.left = element_blank(), 
        axis.text.y.left = element_blank(),
        axis.title.y.right = element_text(face = 'bold', size = 14),
...
## ES Mpa vs. ES LFM
```{r}
ES.lfm.vs.mpa <- function(df, x, y, std.error.x, std.error.y, limits){
...
tti.ES.lfm.vs.mpa <- ES.lfm.vs.mpa(df = tti.ES.df, x = tti.ES.mpa, y = tti.ES.lfm,
                                   ...
                                   limits = c(-5.5, 12))
  #annotate(geom = 'text', x = 2.53, y = 10, label = 'b', size = 5)
tti.ES.lfm.vs.mpa

fh.ES.lfm.vs.mpa <- ES.lfm.vs.mpa(df = fh.ES.df, x = fh.ES.mpa, y = fh.ES.lfm,
                                   std.error.x = fh.SE.mpa, std.error.y = fh.SE.lfm,
                                   limits = c(-8.5, 1.5))
  #annotate(geom = 'text', x = -2, y = -1.5, label = 'c', size = 5) +
  #annotate(geom = 'text', x = -0.2, y = -1, label = 'c', size = 5)
fh.ES.lfm.vs.mpa

temp_max.ES.lfm.vs.mpa <- ES.lfm.vs.mpa(df = temp_max.ES.df, x = temp_max.ES.mpa,
                      y = temp_max.ES.lfm, std.error.x = temp_max.SE.mpa,
                      std.error.y = temp_max.SE.lfm, limits = c(-27, 27))
  #annotate(geom = 'text', x = 19, y = 6.5, label = 'a', size = 5)
temp_max.ES.lfm.vs.mpa

ggarrange(tti.ES.lfm.vs.mpa, fh.ES.lfm.vs.mpa, temp_max.ES.lfm.vs.mpa, ncol = 3, common.legend = T)
```
...
gd.ES.df2 <- gd.ES.df
gd.ES.df2$gd.sig.mpa <- factor(gd.ES.df2$gd.sig.mpa, levels = c("Yes", "No"))

plot_legend.ES <- ggplot(data = gd.ES.df2, aes(x = species, y = gd.ES.mpa,
                                                color = species, shape = gd.sig.mpa)) +
  labs(y = '', color = 'Species', shape = 'Significant?') +
  geom_point(size = 4, cex = 6, alpha = 0.9) +
  geom_linerange(aes(ymin = gd.ES.mpa - 0.5*gd.SE.mpa, ymax = gd.ES.mpa + 0.5*gd.SE.mpa), size = 0.8) +
  theme_bw() +
  #color_many2 +
  color_noceco+
  scale_shape_manual(values = c(16, 10)) +
...
ES.plots <- cowplot::plot_grid(tti.ES.lfm.plot, tti.ES.mpa.plot,
                              ...
                              ncol = 2, nrow = 6,
                              rel_widths = c(6,7.5),
                              rel_heights = c(11.2, 1.3, 10.25, 1, 9.6, 1.8))
...
legend_and_boxplot <- cowplot::plot_grid(blank_plot, boxplot.no.tlp.split, blank_plot, legend, ncol = 1, rel_heights = c(0.3,4,0.5,6))
