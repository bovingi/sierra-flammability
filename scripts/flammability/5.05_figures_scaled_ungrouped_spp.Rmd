...
temp_change_plot_lfm_r<- mem_data_all_r %>% 
  ...
  geom_smooth(method = "lm", se = F, aes(color = F.Group,
                   lty = F.temp.max.lfm.sig), alpha = .75, lty = 3) +
  scale_y_continuous(limits = c(-15, 250), breaks = c(0, 50, 100, 150, 200, 250)) +
  theme(legend.position = "none",
                       axis.text = element_text(size = 13),
                       axis.title = element_text(size = 16, face = 'bold'))+
  #color_many2 + # For color by spp w/ CECO
  ...
  labs(x = "LFM (scaled, centered)", 
       y = "Temp. Change (C)") +
  annotate(geom = "text", x = -2.75, y = 250, label = 'd', fontface = 'bold', size = 10) +
  annotate(geom = 'text', x = -2.3, y = -12, label = 'dry', size = 5) +
  annotate(geom = 'text', x = 3.05, y = -12, label = 'wet', size = 5) +
  geom_segment(aes(x = -2, y = -12, xend = 2.75, yend = -12),
     arrow = arrow(ends = 'both', length = unit(1/2, "picas")), 
      size = .5, alpha = 0.8)
temp_change_plot_lfm_r
...
# 2.5. Significance of interaction terms: 
...
## Temp change
```{r}
tmp_mod_lfm <- lmer(temp_change ~ spp*lfm_scaled +  site + year_month  + (1 | individual), data = mem_data_all_r)
em_temp_change_lfm <- emmeans::emtrends(tmp_mod_lfm, pairwise~spp, var = c("lfm_scaled"))
...
```{r}
tmp_mod_lfm <- lmer(temp_change ~ F.Group*lfm_scaled +  site + year_month  + (1 | individual), data = mem_data_all_r)
em_temp_change_lfm <- emmeans::emtrends(tmp_mod_lfm, pairwise~F.Group, var = c("lfm_scaled"))
summary(em_temp_change_lfm, infer=c(TRUE,TRUE),null=0)
```

```{r}
tmp_mod_mpa <- lmer(temp_change ~ spp*mpa_scaled +  site + year_month  + (1 | individual), data = mem_data_all_r)
em_temp_change_mpa <- emmeans::emtrends(tmp_mod_mpa, pairwise~spp, var = "mpa_scaled")
...
```{r}
tmp_mod_mpa <- lmer(temp_change ~ F.Group*mpa_scaled +  site + year_month  + (1 | individual), data = mem_data_all_r)
em_temp_change_mpa <- emmeans::emtrends(tmp_mod_mpa, pairwise~F.Group, var = "mpa_scaled")
...
#   `(Intercept)` = "Intercept",
#   lfm_scaled = "LFM",
#  mpa_scaled = "Water Pot.", 
#   sample_wt_scaled = "Sample wt.", 
#   site = "Site",
#   sppabco = "A. Concolor",
#  spparpa = "A. patula",
#  sppcade = "C. decurrens", 
#  spppije = "P. jeffreyi", 
#  sppquke = "Q. kellogii", 
#  sppceco = "C. cordulatus"
#  # appcade:mpa_scaled = ""
#  #  `spparpa:lfm_scaled` = "ARPA:LFM"
#  # c172code3 = "Education: high (Carer)",
# #  a_non_used_label = "We don't care"
# )

tab_model(tti_mod, fh_mod, gd_mod, lower_temp_max_mod
         # , prop_ignite_mod
          ,
#           terms = c("`(Intercept)`","lfm_scaled","mpa_scaled","sample_wt_scaled","site", 
#                     "sppabco",
# "spparpa",
#  "sppcade",
#  "spppije",
...
species <- c("A. concolor", "P. jeffreyi", "C. decurrens", "A. patula", "Q. kelloggii")
ref <- temp_mod.coef.df$estimate[6]
temp_change.ES.lfm <- c(ref, ref + temp_mod.coef.df$estimate[12],
                ref + temp_mod.coef.df$estimate[11],
                ref + temp_mod.coef.df$estimate[10],
                ref + temp_mod.coef.df$estimate[13])
  # Following this order: ABCO, PIJE, CADE, ARPA, QUKE (no CECO)
temp_change.SE.lfm <- c(temp_mod.coef.df$std.error[c(6, 12, 11, 10, 13)])
...
ref2 <- temp_mod.coef.df$estimate[21]
temp_change.ES.mpa <- c(ref2, ref2 + temp_mod.coef.df$estimate[27],
                ref2 + temp_mod.coef.df$estimate[26], 
                ref2 + temp_mod.coef.df$estimate[25], 
                ref2 + temp_mod.coef.df$estimate[28]) 
temp_change.SE.mpa <- c(temp_mod.coef.df$std.error[c(21, 27, 26, 25, 28)])
...
Setup:
```{r}
coef <- c("Intercept", "Live Fuel Moisture (LFM)", "Water Potential (MPa)", "Sample Weight", "Site", "Ab. concolor", "Pi. jeffreyii", "Ca. decurrens", "Ar. patula", "Qu. kelloggii", "Interaction Effects", "Pi. jeffreyii x LFM", "Pi. jeffreyii x MPa", "Ca. decurrens x LFM", "Ca. decurrens x MPa", "Ar. patula x LFM", "Ar. patula x MPa", "Qu. kelloggii x LFM", "Qu. kelloggii x MPa", "October 2020", "September 2020", "Random Effects", "Sigma Squared", "Tau00", "N", "Observations", "Marginal Rsq / Conditional Rsq", "AIC")
tti.est <- c(tti_est.df$estimate[c(1,7,6,10,8)], 'Reference', tti_est.df$estimate[c(4,3,2,5)], ' ', tti_est.df$estimate[c(17,13,16,12,15,11, 18, 13)], 'Reference', tti_est.df$estimate[9], ' ', '440.16', '64.43individual', '101individual', '580', '0.578/0.632', '5184.092')
tti.p <- c(tti_est.df$p.value[c(1,7,6,10,8)], 1, tti_est.df$p.value[c(4,3,2,5)], 1, tti_est.df$p.value[c(17,13,16,12,15,11, 18, 13)], 1, tti_est.df$p.value[9], 1, 1, 1, 1, 1, 1, 1)
fh.est <- c(fh_est.df$estimate[c(1,7,6,10,8)], 'Reference', fh_est.df$estimate[c(4,3,2,5)], ' ', fh_est.df$estimate[c(17,13,16,12,15,11, 18, 13)], 'Reference', fh_est.df$estimate[9], ' ', '34.90', '6.31individual', '101individual', '580', '0.272/0.383', '3768.649')
fh.p <- c(fh_est.df$p.value[c(1,7,6,10,8)], 1, fh_est.df$p.value[c(4,3,2,5)], 1, fh_est.df$p.value[c(17,13,16,12,15,11, 18, 13)], 1, fh_est.df$p.value[9], 1, 1, 1, 1, 1, 1, 1)
mt.est <- c(mt_est.df$estimate[c(1,7,6,10,8)], 'Reference', mt_est.df$estimate[c(4,3,2,5)], ' ', mt_est.df$estimate[c(17,13,16,12,15,11, 18, 13)], 'Reference', mt_est.df$estimate[9], ' ', '3614.55', '167.75individual', '101individual', '580', '0.238/0.383', '6334.413')
mt.p <- c(mt_est.df$p.value[c(1,7,6,10,8)], 1, mt_est.df$p.value[c(4,3,2,5)], 1, mt_est.df$p.value[c(17,13,16,12,15,11, 18, 13)], 1, mt_est.df$p.value[9], 1, 1, 1, 1, 1, 1, 1)
gd.est <- c(gd_est.df$estimate[c(1,7,6,10,8)], 'Reference', gd_est.df$estimate[c(4,3,2,5)], ' ', gd_est.df$estimate[c(17,13,16,12,15,11, 18, 13)], 'Reference', gd_est.df$estimate[9], ' ', '1780.64', '16.61individual', '101individual', '580', '0.312/0.319', '5920.094')
gd.p <- c(gd_est.df$p.value[c(1,7,6,10,8)], 1, gd_est.df$p.value[c(4,3,2,5)], 1, gd_est.df$p.value[c(17,13,16,12,15,11, 18, 13)], 1, gd_est.df$p.value[9], 1, 1, 1, 1, 1, 1, 1)
mem.table.df <- data.frame(coef, tti.est, tti.p, fh.est, fh.p, mt.est, mt.p, gd.est, gd.p)
```

Kable:
```{r}
mem.table.df %>%
  mutate(coef = cell_spec(coef, 'html', bold = ifelse(coef == c("Interaction Effects", "Random Effects"), T, F), italic = ifelse(coef == c("Ab. concolor", "Pi. jeffreyii", "Ca. decurrens", "Ar. patula", "Qu. kelloggii"), T, F), color = 'black')) %>% 
  ...
  mutate(mt.est = cell_spec(mt.est, 'html', bold = ifelse(mt.p < 0.05, T, F), color = 'black')) %>% 
  mutate(gd.est = cell_spec(gd.est, 'html', bold = ifelse(gd.p < 0.05, T, F), color = 'black')) %>% 
  select(coef, tti.est, fh.est, mt.est, gd.est) %>% 
  kable(format = 'html', escape = F, digits = 5, col.names = c('Coefficients', 'Time to Ignition', 'Flame Height', 'Maximum Temperature', 'Glow Duration')) %>% 
  kable_styling(bootstrap_options = c('hover', 'bordered', 'condensed'), fixed_thead = T, font_size = 30) %>% 
  row_spec(0, bold = T, color = 'black') %>% 
  row_spec(c(6, 20), italic = T) %>% 
  row_spec(c(11,22), background = '#D3D3D3') %>% 
