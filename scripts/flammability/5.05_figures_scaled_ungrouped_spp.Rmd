...
emtrends_tti_lfm <- tti_lfm$emtrends %>% 
  as.data.frame() %>% 
   mutate(spp = forcats::fct_relevel(spp, "abco", 
                                     "pije", 
                                     "cade", 
                                     "arpa", 
                                     "quke"
...
emtrends_tti_lfm_list <- emtrends_tti_lfm %>% select(p.value) %>% as.list()
...
Problem: does not apply function to anything beyond the first value. 
...
tti.sig.lfm.new <- significance(tti.p.value.lfm.new, tti.sig.lfm.new)
```
...
tti.p.value.lfm.new_df <- tti.p.value.lfm.new %>% as.data.frame()

tti.sig.lfm.new <- significance(tti.p.value.lfm.new_df, tti.sig.lfm.new)
tti.sig.lfm.new <- significance(tti.p.value.lfm.new, tti.sig.lfm.new)
```
...
#new
tti.p.value.lfm.new <- c(emtrends_tti_lfm_list) %>% unlist() %>% unname()
