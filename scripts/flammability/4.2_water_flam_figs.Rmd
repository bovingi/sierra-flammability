...
mem_data_all <- read_csv(here("processed-data", "sierra_flam_data_all.csv"), show_col_types = FALSE) %>% 
  select(lfm, mpa, tti, fh, fd, gd, prop_ignite, temp_change, ignition, sample_wt, dry_wt, prop_new, fresh_wt, water_wt, location, site, year_month, spp, individual) %>% 
...
tti_df_lfm1 <- field_data_all_means %>% 
  filter(age == "old") %>% 
  modelr::add_predictions(tti_mod_lfm) %>% 
   drop_na(pred) 

# To get standard deviations
tti_df_stdev_lfm_old <- field_data_all_means  %>% 
  drop_na(lfm_scaled, sample_wt_scaled, spp) %>% 
  add_predictions(tti_mod_lfm)

tti_df_stdev_lfm_old <- tti_df_stdev_lfm_old %>% 
  drop_na(pred) %>% 
  group_by(spp, month) %>% 
  mutate(stdev.pred = sd(pred))

tti_df_lfm_old <- tti_df_lfm1 %>% 
  mutate(stdev.pred = tti_df_stdev$stdev.pred) %>%
  group_by(spp, month) %>% 
