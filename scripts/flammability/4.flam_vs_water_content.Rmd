---
title: "Flammability vs. Water Content"
...
# install.packages("cowplot", "nanair")
...
#write.csv(mem_data_all, here("processed-data", "mem_final_from5.05.csv"))
...
mem_data_all <- mem_data_all %>% 
  mutate(tti.mpa.sig = case_when(
    spp == "arpa" ~ "Yes", spp == "abco" ~ "Yes", spp == "cade" ~ "No", spp == "ceco" ~ "No", 
    spp == "pije" ~ "Yes", spp == "quke" ~ "Yes")) %>% 
  mutate(tti.lfm.sig = case_when(
    spp == "arpa" ~ "No", spp == "abco" ~ "Yes", spp == "cade" ~ "No", spp == "ceco" ~ "No", 
    spp == "pije" ~ "Yes", spp == "quke" ~ "Yes")) %>% 
  mutate(fh.sig = case_when(
    spp == "arpa" ~ "No", spp == "abco" ~ "No", spp == "cade" ~ "Yes", spp == "ceco" ~ "No", 
    spp == "pije" ~ "No", spp == "quke" ~ "Yes")) %>% 
  mutate(fd.sig = case_when(
    spp == "arpa" ~ "No", spp == "abco" ~ "No", spp == "cade" ~ "No", spp == "ceco" ~ "No", 
    spp == "pije" ~ "No", spp == "quke" ~ "No")) %>%  
  mutate(gd.mpa.sig = case_when(
    spp == "arpa" ~ "No", spp == "abco" ~ "No", spp == "cade" ~ "No", spp == "ceco" ~ "Yes", 
    spp == "pije" ~ "No", spp == "quke" ~ "No")) %>% 
  mutate(gd.lfm.sig = case_when(
    spp == "arpa" ~ "No", spp == "abco" ~ "No", spp == "cade" ~ "No", spp == "ceco" ~ "Yes", 
    spp == "pije" ~ "No", spp == "quke" ~ "Yes")) %>% 
  ...
  mutate(temp.max.sig = case_when(
      spp == "arpa" ~ "Yes", spp == "abco" ~ "No", spp == "cade" ~ "No",
      spp == "ceco" ~ "No", spp == "pije" ~ "Yes", spp == "quke" ~ "Yes"
    ))
...
mem_data_all <- mem_data_all %>% 
  ...
  mutate(F.fh.sig.lfm = case_when(
    F.Group == "Angiosperm" ~ "Yes", F.Group == "Gymnosperm" ~ "Yes")) %>% 
   mutate(F.fh.sig.mpa = case_when(
    F.Group == "Angiosperm" ~ "Yes", F.Group == "Gymnosperm" ~ "No")) %>% 
  mutate(F.gd.sig = case_when(
    F.Group == "Angiosperm" ~ "No", F.Group == "Gymnosperm" ~ "No")) %>%  # Note: the significance here is out of date
  mutate(F.temp.max.sig = case_when(
    F.Group == "Angiosperm" ~ "No", F.Group == "Gymnosperm" ~ "Yes")) %>% 
  mutate(F.temp.change.sig = case_when(
     F.Group == "Angiosperm" ~ "No", F.Group == "Gymnosperm" ~ "No")) %>%
  mutate(F.fd.sig = case_when(
    F.Group == "Angiosperm" ~ "Yes", F.Group == "Gymnosperm" ~ "No")) # Note: the significance hrrere is out of date
```
...
```{r}
tti_mod.lfm <- lmer(tti ~ spp*lfm_scaled 
                    + sample_wt_scaled 
                    + (1 | individual), data = mem_data_all_r)

tti_mod.mpa <- lmer(tti ~ spp*mpa_scaled 
                    + sample_wt_scaled 
                    + (1 | individual), data = mem_data_all_r)

tti_mod_fgroup <- lmer(tti ~ F.Group*mpa_scaled + F.Group*lfm_scaled + year_month + sample_wt_scaled + (1|individual), data = mem_data_all_r)
```

Remef:
```{r}
r_tti_mpa <- remef(tti_mod_fgroup, fix = c("sample_wt_scaled", "lfm_scaled"), ran = list(individual = c("(Intercept)"))) # Using Remef for plots with MPa
r_tti_lfm <- remef(tti_mod_fgroup, fix = c("mpa_scaled", "sample_wt_scaled"), ran = list(individual = c("(Intercept)"))) # Using Remef for plots with LFM
...
Models:
```{r}
fh_mod <- lmer(fh ~ spp*mpa_scaled + spp*lfm_scaled +  year_month + sample_wt_scaled + (1 | individual), data = mem_data_all_r)

fh_mod.lfm <- lmer(fh ~ spp*lfm_scaled + year_month + sample_wt_scaled + (1 | individual), data = mem_data_all_r)
fh_mod.mpa <- lmer(fh ~ spp*mpa_scaled + year_month + sample_wt_scaled + (1 | individual), data = mem_data_all_r)

fh_mod_fgroup <- lmer(fh ~ F.Group*mpa_scaled + F.Group*lfm_scaled + year_month + sample_wt_scaled + (1|individual), data = mem_data_all_r)
```

Remef:
```{r}
r_fh_mpa <- remef(fh_mod_fgroup, fix = c("sample_wt_scaled", "lfm_scaled"), ran = list(individual = c("(Intercept)"))) # Using Remef for plots with MPa
r_fh_lfm <- remef(fh_mod_fgroup, fix = c("mpa_scaled", "sample_wt_scaled"), ran = list(individual = c("(Intercept)"))) # Using Remef for plots with LFM
...
fh_plot_r <- mem_data_all_r %>% 
  ggplot(aes(y = r_fh_mpa, 
             ...
             group = F.Group)) +
  geom_point(alpha= .4, aes(color = F.Group))+
  geom_smooth(method = "lm", se = F, aes(color = F.Group, lty = F.fh.sig.mpa), alpha = .75) +
  scale_color_manual(values = c('steelblue3', 'darkgreen'))+
  theme(legend.position = "none",
        axis.text = element_text(size = 14),
        axis.title = element_text(size = 22, face = 'bold'))+
...
fh_plot_lfm_r <- mem_data_all_r %>% 
  ggplot(aes(y = r_fh_lfm, 
             ...
             group = F.Group)) +
  geom_point(alpha= .4, aes(color = F.Group))+
  geom_smooth(method = "lm", se = F, aes(color = F.Group, lty = F.fh.sig.lfm), alpha = .75) +
  scale_color_manual(values = c('steelblue3', 'darkgreen')) + 
...
Models:
```{r}
lower_temp_max_mod <- lmer(lower_temp_max ~ spp*mpa_scaled + spp*lfm_scaled +  year_month + sample_wt_scaled + (1 | individual), data = mem_data_all_r)

lower_temp_max_mod.lfm <- lmer(lower_temp_max ~ spp*lfm_scaled + year_month + sample_wt_scaled + (1 | individual), data = mem_data_all_r)
lower_temp_max_mod.mpa <- lmer(lower_temp_max ~ spp*mpa_scaled + year_month + sample_wt_scaled + (1 | individual), data = mem_data_all_r)

lower_temp_max_mod_fgroup <- lmer(lower_temp_max ~ F.Group*mpa_scaled + F.Group*lfm_scaled + year_month + sample_wt_scaled + (1|individual), data = mem_data_all_r)
```

Remef:
```{r}
r_lower_temp_max_mpa <- remef(lower_temp_max_mod_fgroup, fix = c("sample_wt_scaled", "lfm_scaled"), ran = list(individual = c("(Intercept)"))) # Using Remef for plots with MPa
r_lower_temp_max_lfm <- remef(lower_temp_max_mod_fgroup, fix = c("mpa_scaled", "sample_wt_scaled"), ran = list(individual = c("(Intercept)"))) # Using Remef for plots with LFM
...
lower_temp_max_plot_lfm_r <- mem_data_all_r %>% 
  ...
  geom_smooth(method = "lm", se = F, aes(color = F.Group,
                   lty = F.temp.max.sig), alpha = .75, lty = 3) +
...
Models:
```{r}
gd_mod <- lmer(gd ~ spp*mpa_scaled + spp*lfm_scaled +  year_month + sample_wt_scaled + (1 | individual), data = mem_data_all_r)

gd_mod.lfm <- lmer(gd ~ spp*lfm_scaled + year_month + sample_wt_scaled + (1 | individual), data = mem_data_all_r)
gd_mod.mpa <- lmer(gd ~ spp*mpa_scaled + year_month + sample_wt_scaled + (1 | individual), data = mem_data_all_r)

gd_mod_fgroup <- lmer(gd ~ F.Group*mpa_scaled + F.Group*lfm_scaled + year_month + sample_wt_scaled + (1|individual), data = mem_data_all_r)
```

Remef:
```{r}
r_gd_mpa <- remef(gd_mod_fgroup, fix = c("sample_wt_scaled", "lfm_scaled"), ran = list(individual = c("(Intercept)"))) # Using Remef for plots with MPa
r_gd_lfm <- remef(gd_mod_fgroup, fix = c("mpa_scaled", "sample_wt_scaled"), ran = list(individual = c("(Intercept)"))) # Using Remef for plots with LFM
...
gd_plot_r <- mem_data_all_r %>% 
  ggplot(aes(y = r_gd_mpa, 
             ...
             group = F.Group)) +
  geom_point(alpha= .4, aes(color = F.Group))+
  geom_smooth(method = "lm", se = F, aes(color = F.Group, lty = F.gd.sig), alpha = .75) +
  scale_color_manual(values = c('steelblue3', 'darkgreen'))+
  theme(legend.position = "none",
        axis.text = element_text(size = 14),
        axis.title = element_text(size = 22, face = 'bold'))+
...
gd_plot_lfm_r <- mem_data_all_r %>% 
  ggplot(aes(y = r_gd_lfm, 
             ...
             group = F.Group)) +
  geom_point(alpha= .4, aes(color = F.Group))+
  geom_smooth(method = "lm", se = F, aes(color = F.Group, lty = F.gd.sig),
              alpha = .75, lty = 3) +
  theme(legend.position = "none",
                       axis.text = element_text(size = 13),
                       axis.title = element_text(size = 16, face = 'bold'))+
...
Models:
```{r}
fd_mod <- lmer(fd ~ spp*mpa_scaled + spp*lfm_scaled +  year_month + sample_wt_scaled + (1 | individual), data = mem_data_all_r)

fd_mod.lfm <- lmer(fd ~ spp*lfm_scaled + year_month + sample_wt_scaled + (1 | individual), data = mem_data_all_r)
fd_mod.mpa <- lmer(fd ~ spp*mpa_scaled + year_month + sample_wt_scaled + (1 | individual), data = mem_data_all_r)

fd_mod_fgroup <- lmer(fd ~ F.Group*mpa_scaled + F.Group*lfm_scaled + year_month + sample_wt_scaled + (1|individual), data = mem_data_all_r)
```

Remef:
```{r}
r_fd_mpa <- remef(fd_mod, fix = c("sample_wt_scaled", "lfm_scaled"), ran = list(individual = c("(Intercept)"))) # Using Remef for plots with MPa
r_fd_lfm <- remef(fd_mod, fix = c("mpa_scaled", "sample_wt_scaled"), ran = list(individual = c("(Intercept)"))) # Using Remef for plots with LFM
...
Models:
```{r}
temp_change_mod <- lmer(temp_change ~ spp*mpa_scaled + spp*lfm_scaled +  year_month + sample_wt_scaled + (1 | individual), data = mem_data_all_r) 

temp_change_mod.fgroup <- lmer(temp_change ~ F.Group*mpa_scaled + F.Group*lfm_scaled +  year_month + sample_wt_scaled + (1 | individual), data = mem_data_all_r)

temp_change_mod.lfm <- lmer(temp_change ~ spp*lfm_scaled + year_month 
                    + sample_wt_scaled 
                    + (1 | individual), data = mem_data_all_r)

temp_change_mod.mpa <- lmer(temp_change ~ spp*mpa_scaled + year_month 
                    + sample_wt_scaled 
                    + (1 | individual), data = mem_data_all_r)
...
Remef:
```{r}
r_temp_change_mpa <- remef(temp_change_mod, fix = c("sample_wt_scaled", "lfm_scaled"), ran = list(individual = c("(Intercept)"))) # Using Remef for plots with MPa
r_temp_change_lfm <- remef(temp_change_mod, fix = c("mpa_scaled", "sample_wt_scaled"), ran = list(individual = c("(Intercept)"))) # Using Remef for plots with LFM
...
temp_change_plot_r <- mem_data_all_r %>% 
  ggplot(aes(y = r_temp_change_mpa, 
             ...
             group = F.Group)) +
  geom_point(alpha= .4, aes(color = F.Group))+
  geom_smooth(method = "lm", se = F, aes(color = F.Group, lty = F.temp.change.sig), alpha = .75) + 
...
temp_change_plot_lfm_r<- mem_data_all_r %>% 
  ...
  geom_point(alpha= .4, aes(color = F.Group))+
  geom_smooth(method = "lm", se = F, aes(color = F.Group,
                   lty = F.temp.change.sig), alpha = .75, lty = 3) +
...
Model:
```{r}
tti_mod_spp <- lmer(tti ~ spp*mpa_scaled + spp*lfm_scaled + year_month + sample_wt_scaled + (1|individual), data = mem_data_all_r)
```

Only species: 

```{r}
tti_mod_spp_only <- lmer(tti ~ spp + spp + year_month + sample_wt_scaled + (1|individual), data = mem_data_all_r)
```

Remef:
```{r}
r_tti_mpa_spp <- remef(tti_mod_spp, fix = c("sample_wt_scaled", "lfm_scaled"), ran = list(individual = c("(Intercept)"))) # Using Remef for plots with MPa
r_tti_lfm_spp <- remef(tti_mod_spp, fix = c("mpa_scaled", "sample_wt_scaled"), ran = list(individual = c("(Intercept)"))) # Using Remef for plots with LFM
...
tti_plot_r_spp <- mem_data_all_r %>% 
  ...
  geom_smooth(method = "lm", se = F, aes(color = Species#, lty = tti.mpa.sig
                                         ), alpha = .75) + 
  scale_y_continuous(limits = c(0, 250), breaks = c(0, 50, 100, 150, 200)) +
  color_noceco+ # For color by species w/o CECO
  scale_linetype_manual(values = c(3, 1)) +
  theme(legend.position = "none", 
        axis.text = element_blank(),#element_text(size = 13),
        axis.title = element_blank(),#element_text(size = 16, face = 'bold'),
        axis.ticks = element_blank()) + 
  labs(x = "", 
       y = "Time to Ignition") +
  annotate(geom = "text", x = -2.9, y = 225, label = 'f', fontface = 'bold', size = 10)
tti_plot_r_spp
...
tti_plot_lfm_r_spp <- mem_data_all_r %>%
  ggplot(aes(y = r_tti_lfm_spp, 
             ...
             group = spp)) +
  geom_point(alpha= .4, aes(color = Species))+
  geom_smooth(method = "lm", se = F, aes(color = Species#, lty = tti.lfm.sig
                                         ), alpha = .75) + 
  scale_y_continuous(limits = c(0, 250), breaks = c(0, 50, 100, 150, 200)) +
  ...
  labs(x = "", 
       y = "Time to Ignition (s)") +
  annotate(geom = "text", x = -2.4, y = 225, label = 'a', fontface = 'bold', size = 10)
tti_plot_lfm_r_spp
```
...
Model:
```{r}
fh_mod_spp <- lmer(fh ~ spp*mpa_scaled + spp*lfm_scaled + year_month + sample_wt_scaled + (1|individual), data = mem_data_all_r)
```

Remef:
```{r}
r_fh_mpa_spp <- remef(fh_mod_spp, fix = c("sample_wt_scaled", "lfm_scaled"), ran = list(individual = c("(Intercept)"))) # Using Remef for plots with MPa
r_fh_lfm_spp <- remef(fh_mod_spp, fix = c("mpa_scaled", "sample_wt_scaled"), ran = list(individual = c("(Intercept)"))) # Using Remef for plots with LFM
...
fh_plot_r_spp <- mem_data_all_r %>% 
  ggplot(aes(y = r_fh_mpa_spp, 
             ...
             group = spp)) +
  geom_point(alpha= .4, aes(color = Species))+
  geom_smooth(method = "lm", se = F, aes(color = Species#, lty = fh.sig
                                         ), alpha = .75) + 
  scale_y_continuous(limits = c(8, 70), breaks = c(10, 20, 30, 40, 50, 60)) +
  color_noceco+ # For color by species w/o CECO
  scale_linetype_manual(values = c(3, 1)) +
  theme(legend.position = "none",
        axis.ticks = element_blank(),
  ...
  labs(x = "", 
       y = "Flame Height (cm)") +
  annotate(geom = "text", x = -2.9, y = 60, label = 'g', fontface = 'bold', size = 10)
fh_plot_r_spp
...
fh_plot_lfm_r_spp <- mem_data_all_r %>%
  ...
  theme(legend.position = "none",
                       ...
                       axis.title.x = element_blank(),
                       axis.text.x = element_blank(), axis.ticks.x = element_blank(),
                       plot.margin = unit(c(0.17, 0.17, 0.17, 0.44), 'cm')) +
  labs(x = "", 
       y = "Flame Height (cm)") +
  annotate(geom = "text", x = -2.4, y = 60, label = 'b', fontface = 'bold', size = 10)
fh_plot_lfm_r_spp
```

No Remef:
```{r}
fh_plot_spp <- mem_data_all %>% 
  ggplot(aes(y = fh, 
             x = mpa_scaled, 
             group = spp)) +
  geom_point(alpha= .4, aes(color = Species))+
  ...
  labs(x = "", 
       y = "Flame Height (cm)") +
  annotate(geom = "text", x = -2.9, y = 64, label = 'd', fontface = 'bold', size = 10)
fh_plot_spp

# For MAIN figure:
fh_plot_lfm_spp <- mem_data_all %>%
  ggplot(aes(y = fh, 
             x = lfm_scaled, 
             group = spp)) +
  geom_point(alpha= .4, aes(color = Species))+
  ...
  labs(x = "", 
       y = "Flame Height (cm)") +
  annotate(geom = "text", x = -2.4, y = 64, label = 'a', fontface = 'bold', size = 10)
fh_plot_lfm_spp
```
...
Model:
```{r}
lower_temp_max_mod_spp <- lmer(lower_temp_max ~ spp*mpa_scaled + spp*lfm_scaled + year_month + sample_wt_scaled + (1|individual), data = mem_data_all_r)
```

Remef:
```{r}
r_lower_temp_max_mpa_spp <- remef(lower_temp_max_mod_spp, fix = c("sample_wt_scaled", "lfm_scaled"), ran = list(individual = c("(Intercept)"))) # Using Remef for plots with MPa
r_lower_temp_max_lfm_spp <- remef(lower_temp_max_mod_spp, fix = c("mpa_scaled", "sample_wt_scaled"), ran = list(individual = c("(Intercept)"))) # Using Remef for plots with LFM
...
lower_temp_max_plot_r_spp <- mem_data_all_r %>% 
  ggplot(aes(y = r_lower_temp_max_mpa_spp, 
             ...
             group = spp)) +
  geom_point(alpha= .4, aes(color = Species))+
  geom_smooth(method = "lm", se = F, aes(color = Species#, lty = temp.max.sig
                                         ), alpha = .75) + 
  color_noceco+ # For color by species w/o CECO
  scale_linetype_manual(values = c(3, 1)) +
  theme(legend.position = "none",                                   
  ...
  labs(x = "", 
       y = "Maximum Temperature (C)") +
  annotate(geom = "text", x = -2.9, y = 480, label = 'i', fontface = 'bold', size = 10)
...
No Remef:
```{r}
lower_temp_max_plot_spp <- mem_data_all %>% 
  ...
  labs(x = "", 
       y = "Maximum Temperature (C)") +
  annotate(geom = "text", x = -2.9, y = 495, label = 'e', fontface = 'bold', size = 10)
lower_temp_max_plot_spp

# For MAIN figure:
lower_temp_max_plot_lfm_spp <- mem_data_all %>%
  ggplot(aes(y = lower_temp_max, 
             x = lfm_scaled, 
             group = spp)) +
  geom_point(alpha= .4, aes(color = Species))+
  ...
  labs(x = "", 
       y = "Maximum Temperature (C)") +
  annotate(geom = "text", x = -2.4, y = 495, label = 'b', fontface = 'bold', size = 10)
lower_temp_max_plot_lfm_spp
```

### GD
Model:
```{r}
gd_mod_spp <- lmer(gd ~ spp*mpa_scaled + spp*lfm_scaled + year_month + sample_wt_scaled + (1|individual), data = mem_data_all_r)
```

Remef:
```{r}
r_gd_mpa_spp <- remef(gd_mod_spp, fix = c("sample_wt_scaled", "lfm_scaled"), ran = list(individual = c("(Intercept)"))) # Using Remef for plots with MPa
r_gd_lfm_spp <- remef(gd_mod_spp, fix = c("mpa_scaled", "sample_wt_scaled"), ran = list(individual = c("(Intercept)"))) # Using Remef for plots with LFM
...
gd_plot_r_spp <- mem_data_all_r %>% 
  ggplot(aes(y = r_gd_mpa_spp, 
             ...
             group = spp)) +
  geom_point(alpha= .4, aes(color = Species))+
  geom_smooth(method = "lm", se = F, aes(color = Species#, lty = gd.mpa.sig
                                         ), alpha = .75) + 
  color_noceco+ # For color by species w/o CECO
  scale_linetype_manual(values = c(3, 1)) +
  theme(legend.position = "none",                                   
  ...
  labs(x = "Water Potential (scaled)", 
       y = "Glow Duration (s)") +
  annotate(geom = "text", x = -2.9, y = 400, label = 'j', fontface = 'bold', size = 10)
gd_plot_r_spp
...
gd_plot_lfm_r_spp <- mem_data_all_r %>%
  ggplot(aes(y = r_gd_lfm_spp, 
             ...
             group = spp)) +
  geom_point(alpha= .4, aes(color = Species))+
  geom_smooth(method = "lm", se = F, aes(color = Species#, lty = gd.lfm.sig
                                         ), alpha = .75) + 
  color_noceco + # For color by spp w/o CECO
  scale_linetype_manual(values = c(3, 1)) +
  theme(legend.position = "none",
...
gd_plot_spp <- mem_data_all %>% 
  ...
  labs(x = "Water Potential (scaled)", 
       y = "Glow Duration (s)") +
  annotate(geom = "text", x = -2.9, y = 575, label = 'f', fontface = 'bold', size = 10)
gd_plot_spp

gd_plot_lfm_spp <- mem_data_all %>%
  ggplot(aes(y = gd, 
             x = lfm_scaled, 
             group = spp)) +
  geom_point(alpha= .4, aes(color = Species))+
  ...
  labs(x = "Live Fuel Moisture (scaled)", 
       y = "Glow Duration (s)") +
  annotate(geom = "text", x = -2.4, y = 575, label = 'c', fontface = 'bold', size = 10)
gd_plot_lfm_spp
```
...
Model:
```{r}
temp_change_mod_spp <- lmer(temp_change ~ spp*mpa_scaled + spp*lfm_scaled + year_month + sample_wt_scaled + (1|individual), data = mem_data_all_r)
```

Remef:
```{r}
r_temp_change_mpa_spp <- remef(temp_change_mod_spp, fix = c("sample_wt_scaled", "lfm_scaled"), ran = list(individual = c("(Intercept)"))) # Using Remef for plots with MPa
r_temp_change_lfm_spp <- remef(temp_change_mod_spp, fix = c("mpa_scaled", "sample_wt_scaled"), ran = list(individual = c("(Intercept)"))) # Using Remef for plots with LFM
...
temp_change_plot_r_spp <- mem_data_all_r %>% 
  ggplot(aes(y = r_temp_change_mpa_spp, 
             ...
             group = spp)) +
  geom_point(alpha= .4, aes(color = Species))+
  geom_smooth(method = "lm", se = F, aes(color = Species#, lty = temp.change.sig
                                         ) , alpha = .75) + 
  color_noceco+ # For color by species w/o CECO
  scale_linetype_manual(values = c(3, 1)) +
  theme(legend.position = "none",                                   
  ...
  annotate(geom = "text", x = -2.9, y = 220, label = 'h', fontface = 'bold', size = 10)
...
Legend:
```{r}
mem_data_all_r$gd.lfm.sig <- factor(mem_data_all_r$gd.lfm.sig, levels = c("Yes", "No"))
...
ggsave(here('figures', 'extra-figures', 'lfm_flam_metrics.jpeg'), plot = lfm_spp_plots_w_legend, height = 15, width = 8)
```
...
ggsave(here('figures', 'extra-figures', 'mpa_flam_metrics.jpeg'), plot = mpa_spp_plots_w_legend, height = 15, width = 8)
```

#### Both
```{r}
both_spp_plots <- cowplot::plot_grid(tti_plot_lfm_r_spp, tti_plot_r_spp, fh_plot_lfm_r_spp, fh_plot_r_spp, temp_change_plot_lfm_r_spp, temp_change_plot_r_spp, lower_temp_max_plot_lfm_r_spp, lower_temp_max_plot_r_spp, gd_plot_lfm_r_spp, gd_plot_r_spp, ncol = 2, rel_widths = c(12,11))

both_spp_plots_w_legend <- cowplot::plot_grid(both_spp_plots, spp_legend, ncol = 2, rel_widths = c(6, 1))
both_spp_plots_w_legend

ggsave(here('figures', 'supp-figures', 'FigS2.spp_flam_metrics.jpeg'), plot = both_spp_plots_w_legend, height = 15, width = 15)
```

No Remef:
```{r}
both_spp_plots_no_remef <- cowplot::plot_grid(fh_plot_lfm_spp, fh_plot_spp, lower_temp_max_plot_lfm_spp, lower_temp_max_plot_spp, gd_plot_lfm_spp, gd_plot_spp, ncol = 2, rel_widths = c(12,11))

both_spp_plots_no_remef_w_legend <- cowplot::plot_grid(both_spp_plots_no_remef, spp_legend, ncol = 2, rel_widths = c(6, 1))
both_spp_plots_no_remef_w_legend

ggsave(here('figures', 'supp-figures', 'UpdatedFigS2.spp_flam_metrics.jpeg'), plot = both_spp_plots_no_remef_w_legend, height = 10, width = 15)
```
...
## TTI
```{r}
tti_mod_lfm <- lmer(tti ~ spp*lfm_scaled + sample_wt + (1 | individual), data = mem_data_all_r)
...
```{r}
tti_mod_lfm <- lmer(tti ~ F.Group*lfm_scaled +  year_month  + (1 | individual), data = mem_data_all_r)
em_tti_lfm <- emmeans::emtrends(tti_mod_lfm, pairwise~F.Group, var = c("lfm_scaled"))
summary(em_tti_lfm, infer=c(TRUE,TRUE),null=0)
```

```{r}
tti_mod_mpa <- lmer(tti ~ spp*mpa_scaled +  year_month  + (1 | individual), data = mem_data_all_r)
...
```{r}
tti_mod_mpa <- lmer(tti ~ F.Group*mpa_scaled +  year_month  + (1 | individual), data = mem_data_all_r)
...
## FH
```{r}
fh_mod_lfm <- lmer(fh ~ spp*lfm_scaled +  year_month  + (1 | individual), data = mem_data_all_r)
...
```{r}
fh_mod_lfm <- lmer(fh ~ F.Group*lfm_scaled +  year_month  + (1 | individual), data = mem_data_all_r)
em_fh_lfm <- emmeans::emtrends(fh_mod_lfm, pairwise~F.Group, var = c("lfm_scaled"))
summary(em_fh_lfm, infer=c(TRUE,TRUE),null=0)
```

```{r}
fh_mod_mpa <- lmer(fh ~ spp*mpa_scaled +  year_month  + (1 | individual), data = mem_data_all_r)
...
```{r}
fh_mod_mpa <- lmer(fh ~ F.Group*mpa_scaled +  year_month  + (1 | individual), data = mem_data_all_r)
...
## FD
```{r}
fd_mod_lfm <- lmer(fd ~ spp*lfm_scaled +  year_month  + (1 | individual), data = mem_data_all_r)
...
```{r}
fd_mod_lfm <- lmer(fd ~ F.Group*lfm_scaled +  year_month  + (1 | individual), data = mem_data_all_r)
em_fd_lfm <- emmeans::emtrends(fd_mod_lfm, pairwise~F.Group, var = c("lfm_scaled"))
summary(em_fd_lfm, infer=c(TRUE,TRUE),null=0)
```

```{r}
fd_mod_mpa <- lmer(fd ~ spp*mpa_scaled +  year_month  + (1 | individual), data = mem_data_all_r)
...
```{r}
fd_mod_mpa <- lmer(fd ~ F.Group*mpa_scaled +  year_month  + (1 | individual), data = mem_data_all_r)
...
## Max. Temp.
```{r}
tmp_mod_lfm <- lmer(lower_temp_max ~ spp*lfm_scaled +  year_month  + (1 | individual), data = mem_data_all_r)
...
```{r}
tmp_mod_lfm <- lmer(lower_temp_max ~ F.Group*lfm_scaled +  year_month  + (1 | individual), data = mem_data_all_r)
...
```{r}
tmp_mod_mpa <- lmer(lower_temp_max ~ spp*mpa_scaled +  year_month  + (1 | individual), data = mem_data_all_r)
...
```{r}
tmp_mod_mpa <- lmer(lower_temp_max ~ F.Group*mpa_scaled +  year_month  + (1 | individual), data = mem_data_all_r)
...
## Temp Change
```{r}
temp_change_mod_lfm <- lmer(temp_change ~ spp*lfm_scaled +  year_month  + (1 | individual), data = mem_data_all_r)
...
```{r}
#temp_change_mod_lfm <- lmer(temp_change ~ F.Group*lfm_scaled +  year_month  + (1 | individual), data = mem_data_all_r)
...
```{r}
temp_change_mod_mpa <- lmer(temp_change ~ spp*mpa_scaled +  year_month  + (1 | individual), data = mem_data_all_r)
...
```{r}
#temp_change_mod_mpa <- lmer(temp_change ~ F.Group*mpa_scaled +  year_month  + (1 | individual), data = mem_data_all_r)
...
## GD
```{r}
gd_mod_lfm <- lmer(gd ~ spp*lfm_scaled + year_month  + (1 | individual), data = mem_data_all_r)
...
```{r}
gd_mod_mpa <- lmer(gd ~ spp*mpa_scaled + year_month  + (1 | individual), data = mem_data_all_r)
...
ST.df %>% kable(format = 'html', escape = F, col.names = c('Flam. Metric', 'Species', 'Slope',                                'Y Intercept', 'R2 Value')) %>% 
  ...
  save_kable(here('figures', 'extra-figures', 'interspecific.differences.summary.table.html'))
```
...
Setup:
```{r}
coef <- c("Intercept", "Live Fuel Moisture (LFM)", "Water Potential (MPa)", "Sample Weight", "Ab. concolor", "Pi. jeffreyii", "Ca. decurrens", "Ar. patula", "Qu. kelloggii", "October 2020", "September 2020", "Interaction Effects", "Pi. jeffreyii x LFM", "Pi. jeffreyii x MPa", "Ca. decurrens x LFM", "Ca. decurrens x MPa", "Ar. patula x LFM", "Ar. patula x MPa", "Qu. kelloggii x LFM", "Qu. kelloggii x MPa", "Random Effects", paste("\U03C3", "\U00B2"), paste("\U03C4", "\U2080", "\U2080"), "N", "Model Summary Statistics", "Observations", paste("Marginal R", "\U00B2", "/", "Conditional R", "\U00B2"))
...
mem.table.df %>%
  ...
  save_kable(here('figures', 'supp-figures', 'TableS2.MEM_results.html'))
```
...
Setup:
```{r}
coef <- c("Intercept", "Live Fuel Moisture (LFM)", "Water Potential (MPa)", "Sample Weight", "Angiosperm", "Gymnosperm", "October 2020", "September 2020", "Interaction Effects", "Gymnosperm x LFM", "Gymnosperm x MPa", "Random Effects", paste("\U03C3", "\U00B2"), paste("\U03C4", "\U2080", "\U2080"), "N", "Model Summary Statistics", "Observations", paste("Marginal R", "\U00B2", "/", "Conditional R", "\U00B2"))
...
mem.table.df %>%
  ...
  save_kable(here('figures', 'supp-figures', 'TableS3.Functional_Group_MEM_Table.html'))
```
