...
```{r}
quke_lfm <- flam_sierra_df %>% 
  filter(spp == "quke") %>% 
  replace_with_na_at("lfm", ~.x > 190) 

ceco_lfm <- flam_sierra_df  %>% 
  filter(spp == "ceco") #not enough ignitions to do anything really --> filter out ceco?

pije_lfm <- flam_sierra_df %>% 
  filter(spp == "pije") %>% 
  replace_with_na_at("lfm", ~.x < 55)  %>% 
  replace_with_na_at("tti", ~.x > 200) 

cade_lfm <- flam_sierra_df %>% 
  filter(spp == "cade") %>% 
  replace_with_na_at("gd", ~.x > 300)

arpa_lfm <- flam_sierra_df %>% 
  filter(spp == "arpa") %>% #remove 1 row
 replace_with_na_at("tti", ~.x > 300) 

abco_lfm <- flam_sierra_df %>% 
  filter(spp == "abco") %>% 
...
#Write csv
```{r}
flam_sierra_df_final <- bind_rows(plot_lfm_df, plot_mpa_df)
