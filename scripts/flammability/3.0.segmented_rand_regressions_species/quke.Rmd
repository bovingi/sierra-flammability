...
o_mod<-lme(tti~lfm + year_month + site, random=~1|id, data=quke_subset_seg_tti)
...
u_mod <- lm(tti~lfm + year_month + site, data = quke_subset_seg_tti)
...
o_mod<-lme(prop_ignite~lfm + year_month + site, random=~1|id, quke_subset_seg_prop_ignite )
...
o_mod<-lme(gti~lfm + year_month + site, random=~1|id, data=quke_subset_seg_gti)
os_mod<-segmented.default(o_mod, ~lfm, npsi=list(lfm=1))
#summarizing results (note the '_coef' argument)
summary.segmented(os_mod)
...
o_mod<-lme(fh~lfm + year_month + site, random=~1|id, data=quke_subset_seg_fh)
os_mod<-segmented.default(o_mod, ~lfm, npsi=list(lfm=1))
#summarizing results (note the '_coef' argument)
summary.segmented(os_mod)
...
o_mod<-lme(lower_temp_max~lfm + year_month + site, random=~1|id, data=quke_subset_seg_lower_temp_max)
os_mod<-segmented.default(o_mod, ~lfm, npsi=list(lfm=1))
#summarizing results (note the '_coef' argument)
summary.segmented(os_mod)
os_mod
...
o_mod<-lme(lower_temp_max~lfm + year_month + site, random=~1|id, data=quke_subset_seg_temp_change)
os_mod<-segmented.default(o_mod, ~lfm, npsi=list(lfm=1))
#summarizing results (note the '_coef' argument)
summary.segmented(os_mod)
os_mod

rsegplot.lfm(os_mod, quke_subset_seg_temp_change, max.x = quke_max_lfm, y.variable = 'lower_temp_max', y.lab = "Maximum Temperature (C)", tlp = mean_lfm_quke, tlp.lower = lwr_lfm_quke, tlp.upper = upr_lfm_quke)
```
...
o_mod<-lme(fd~lfm + year_month + site, random=~1|id, data=quke_subset_seg_fd)
os_mod_f<-segmented.default(o_mod, ~lfm, npsi=list(lfm=1))
#summarizing results (note the '_coef' argument)
summary.segmented(os_mod_f)

rsegplot.lfm(os_mod_f, quke_subset_seg_fd, max.x = quke_max_lfm, y.variable = 'fd', y.lab = "Flame Duration (s)", tlp = mean_lfm_quke, tlp.lower = lwr_lfm_quke, tlp.upper = upr_lfm_quke)
```
...
## Water Potential
```{r}
quke_subset_seg <- quke_subset_seg %>% 
...
### Time to Ignition
```{r}
o_mod<-lme(tti~mpa + year_month + site, random=~1|id, data=quke_subset_seg_tti)
os_mod_b1<-segmented.default(o_mod, ~mpa, npsi=list(mpa=1))
#summarizing results (note the '_coef' argument)
summary.segmented(os_mod_b1)

rsegplot.mpa(os_mod_b1, quke_subset_seg_tti, max.x = quke_max_mpa, y.variable = 'tti', y.lab = "Time to Ignition (s)", tlp = mean_mpa_quke, tlp.lower = lwr_mpa_quke, tlp.upper = upr_mpa_quke)
```

### Prop_ Ignite (5%)
```{r}
quke_subset_seg_prop_ignite

o_mod<-lme(prop_ignite~mpa + year_month + site, random=~1|id, data=quke_subset_seg_prop_ignite)
...
### Glow to Ignition
```{r}
quke_subset_seg_gti <- quke_subset_seg_gti %>% 
  filter(gti < 50, 
         gti > 0)

o_mod<-lme(gti~mpa + year_month + site, random=~1|id, data=quke_subset_seg_gti)
os_mod<-segmented.default(o_mod, ~mpa, npsi=list(mpa=1))
#summarizing results (note the '_coef' argument)
summary.segmented(os_mod)
...
o_mod<-lme(lower_temp_max~mpa + year_month + site, random=~1|id, quke_subset_seg_lower_temp_max)
os_mod<-segmented.default(o_mod, ~mpa, npsi=list(mpa=1))
#summarizing results (note the '_coef' argument)
summary.segmented(os_mod)

rsegplot.mpa(os_mod, quke_subset_seg_lower_temp_max, max.x = quke_max_mpa, y.variable = 'lower_temp_max', y.lab = "Lower Temp Max (C)", tlp = mean_mpa_quke, tlp.lower = lwr_mpa_quke, tlp.upper = upr_mpa_quke)
```

Combustibility Metrics:
### Flame Height
```{r}
o_mod<-lme(fh~mpa + year_month + site, random=~1|id, data=quke_subset_seg_fh)
os_mod<-segmented.default(o_mod, ~mpa, npsi=list(mpa=1))
#summarizing results (note the '_coef' argument)
summary.segmented(os_mod)
...
### Flame Duration
```{r}
o_mod<-lme(fd~mpa + year_month + site, random=~1|id, data=quke_subset_seg_fd)
os_mod<-segmented.default(o_mod, ~mpa, npsi=list(mpa=1))
#summarizing results (note the '_coef' argument)
summary.segmented(os_mod)

rsegplot.mpa(os_mod, quke_subset_seg_fd, max.x = quke_max_mpa, y.variable = 'fd', y.lab = "Flame Duration (sec)", tlp = mean_mpa_quke, tlp.lower = lwr_mpa_quke, tlp.upper = upr_mpa_quke)
```
# -------
