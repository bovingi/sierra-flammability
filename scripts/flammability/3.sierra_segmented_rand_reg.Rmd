...
```{r}
segmented.fxn <- function(y.var, x1, x2, x3, id, df){
  o_mod <- lme(y.var ~ x1 + x2 + x3, random = ~1|id, data = df)
  os_mod_b<-segmented.default(o_mod, ~x1, npsi=list(x1=1))
  summary.segmented(os_mod_b)
}

seg_data_subset %>% 
  group_by(spp) %>% 
  segmented.fxn(y.var = tti, 
                x1 = lfm, 
                x2 = year_month, 
                x3 = site, 
                id = id)
...
rsegplot.lfm.nolegend(os_mod_b, abco_subset_seg, y.variable = 'tti',y.lab = "Time to Ignition (s)", tlp = mean_lfm_ceme, tlp.lower = lwr_lfm_ceme, tlp.upper = upr_lfm_ceme)
```
