...
sierra_flamdata_all <- read_csv(here("processed-data", "analysis 2.0", "sierra_flam_data_all.csv"), show_col_types = FALSE) %>%
  mutate(LFM = lfm_n_as_imputed) 

sierra_flamdata <- sierra_flamdata_all %>% 
  filter(ignition == 1)

 # filter(year == 2020)

figure_epi_pca_quant <- (sierra_flamdata[, c("fh", "ttfg", "tti", "fd", "gd", "gti", "pfg",
         "temp_max")]) %>%  
...
figure_epi_pca_cat <- (sierra_flamdata[c("hydration", "spp", "year_month", "sample")]) %>% 
...
```{r warning=FALSE}
x_flam <-predict(pcobj)  ### generate scores for each row in sierra_flamdata

pca_flam_data <- as_data_frame(x_flam) 

all_data_epi <- cbind(sierra_flamdata, pca_flam_data)

# #because we wants instances where they didn't ignite for the logistic regression: 
sierra_flamdata_noignitions <- sierra_flamdata %>%
 # mutate(LFM = lfm_NAs_imputed) %>%
  #filter(year == 2020) %>%
 # filter(model == "EPI") %>%
  filter(ignition == 0)

all_data_withnoignitions <- bind_rows(all_data_epi, sierra_flamdata_noignitions)
...
write_csv(mem_data, here("processed-data", "analysis 2.0", "mem_data_sierra_alldates_csv"))
```
...
```{r warning=FALSE}
### Note: This dataset includes only Epiradiator and Ignited samples
sierra_flamdata.adfa <- sierra_flamdata %>% 
  filter(spp == "ADFA")

 # filter(year == 2020)

figure_epi_pca_quant.adfa <- (sierra_flamdata.adfa[, c("fh", "ttfg", "tti", "fd", "gd", "gti", "pfg",
         "temp_max")]) %>%  
 transmute("\n\nFlame Duration" = fd,
           " Flame Height" = fh,
           "Post-flame Glow" = pfg,
...
figure_epi_pca_cat.adfa <- (sierra_flamdata.adfa[c("hydration", "spp", "year_month", "sample")]) %>% 
  transmute("Hydration" = hydration, 
            spp = spp,
            "Timing" = year_month, 
            "Sample" = sample) %>% 
...
```{r warning=FALSE}
### Note: This dataset includes only Epiradiator and Ignited samples
sierra_flamdata.ceme <- sierra_flamdata %>% 
  filter(spp == "CEME")

 # filter(year == 2020)

figure_epi_pca_quant.ceme <- (sierra_flamdata.ceme[, c("fh", "ttfg", "tti", "fd", "gd", "gti", "pfg",
         "temp_max")]) %>%  
...
figure_epi_pca_cat.ceme <- (sierra_flamdata.ceme[c("hydration", "spp", "year_month", "sample")]) %>% 
  transmute("Hydration" = hydration, 
            spp = spp,
            "Timing" = year_month, 
            "Sample" = sample) %>% 
