...
DEC 14 NOTES: Actually we might! 
...
Visualization:
```{r}
ggplot(spp_data_pca, aes(x = PC1, y = PC2, color = F.Group)) +
  geom_point(size = 1.7, alpha = .35) +
  stat_ellipse(aes(x = PC1, y = PC2, color = F.Group), alpha = 0.5, cex = 0.95) +
  labs(color = "Functional Group") +
...
#ggsave(here("figures/PCA.spp.diff.jpg"), height = 8, width = 10) 
# TO RUN SPP PCA: change F.Group > Species, legend.text = element_text(face = italic) and change color palette to color_many2
```
