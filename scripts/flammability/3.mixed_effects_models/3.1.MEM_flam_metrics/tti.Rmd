...
```{r}
spp_only <- lmer(tti ~ spp + site + year_month + (1 | individual), data = mem_data_all)

sppXlfm <- lmer(tti ~ lfm_scaled*spp + site + year_month + sample_wt_scaled + (1 | individual), data = mem_data_all)

sppXmpa <- lmer(tti ~ mpa_scaled*spp + site + year_month + sample_wt_scaled + (1 | individual), data = mem_data_all)

spp_lfm <- lmer(tti ~ lfm_scaled + spp + site + year_month + sample_wt_scaled + (1 | individual), data = mem_data_all)

spp_mpa <- lmer(tti ~ mpa_scaled + spp + site + year_month + sample_wt_scaled + (1 | individual), data = mem_data_all)

spp_sw <- lmer(tti ~ spp + site + year_month + sample_wt_scaled + (1 | individual), data = mem_data_all)
...
tab_model(spp_sw, spp_only, spp_lfm, spp_mpa, sppXlfm, sppXmpa,
  ...
  string.p = "P-Value", 
  p.style = "stars",
  file = here('figures', 'MEM_figures','final_mods_tti.html')) 
```
...
```{r}
m16 <- lmer(tti ~ spp + site + year_month + (1 | individual), data = mem_data_all)

performance::multicollinearity(m16)
```
...
```{r}
tab_model(m7, m11, m12.5, m13, m16,
          show.reflvl = TRUE, 
          digits = 3, 
          show.aic = TRUE, 
          show.ci = FALSE,
...
```{r}
tab_model(m11, sw_m12.5, mspp_wt, m16,
          show.reflvl = TRUE, 
          digits = 3, 
          show.aic = TRUE, 
          show.ci = FALSE,
