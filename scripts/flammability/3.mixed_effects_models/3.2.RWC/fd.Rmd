---
title: "Mixed Effects Models (RWC) - Flame Duration"
...
```{r}
sw_max_model <- lmer(fd ~ spp + dw_flam_sample_scaled + ww_flam_sample_scaled +  mpa_scaled + rwc_scaled + excess_water_scaled + site + year_month + sample_wt_scaled + (1 | individual), data = mem_data_all)

performance::multicollinearity(sw_max_model)

#tidy_fd <- broom.mixed::tidy(sw_max_model)
```

```{r}
sw_max_model_norwc <- lmer(fd ~ spp + dw_flam_sample_scaled +  ww_flam_sample_scaled +  mpa_scaled + excess_water_scaled + site +  year_month + sample_wt_scaled + (1 | individual), data = mem_data_all)

performance::multicollinearity(sw_max_model_norwc)
```

```{r}
sw_max_model_nompa <- lmer(fd ~ spp + dw_flam_sample_scaled +  ww_flam_sample_scaled +  rwc+ excess_water_scaled + site +  year_month + sample_wt_scaled + (1 | individual), data = mem_data_all)
...
```{r}
sw_m7 <- lmer(fd ~ mpa_scaled + rwc_scaled+ spp + site + year_month + sample_wt_scaled + (1 | individual), data = mem_data_all)
AIC(sw_m7)
performance::multicollinearity(sw_m7)
```

```{r}
sw_m10 <- lmer(fd ~ rwc_scaled + spp + site + year_month + sample_wt_scaled + (1 | individual), data = mem_data_all)
AIC(sw_m10)
...
```{r}
sw_m12.5 <- lmer(fd ~ excess_water_scaled + mpa_scaled + rwc_scaled+ spp + site + year_month + sample_wt_scaled + (1 | individual), data = mem_data_all)

performance::multicollinearity(sw_m12.5) ##High correlation! Don't use
```

```{r}
sw_m13 <- lmer(fd ~ excess_water_scaled + rwc_scaled+ spp + site + year_month + sample_wt_scaled + (1 | individual), data = mem_data_all)
...
tab_model(sw_mpa_mod_fig.mpa, 
  ...
  string.p = "P-Value", 
  p.style = "stars",
  file = here('figures', 'MEM_figures','site_yearmonth_samplewt_fd_rwc.html')) # saving _html into figures folder
```
...
```{r}
max_model <- lmer(fd ~ spp + dw_flam_sample_scaled +  ww_flam_sample_scaled +  mpa_scaled + rwc_scaled+ excess_water_scaled + site + year_month + (1 | individual), data = mem_data_all)

performance::multicollinearity(max_model)
```

```{r}
max_model_noww <- lmer(fd ~ spp + dw_flam_sample_scaled +  mpa_scaled + rwc_scaled + excess_water_scaled + site + year_month + (1 | individual), data = mem_data_all)

performance::multicollinearity(max_model_noww)
```

```{r}
max_model_norwc <- lmer(fd ~ spp + dw_flam_sample_scaled +  ww_flam_sample_scaled +  mpa_scaled + excess_water_scaled + site +  year_month + (1 | individual), data = mem_data_all)

performance::multicollinearity(max_model_norwc)
```

```{r}
max_model_nompa <- lmer(fd ~ spp + dw_flam_sample_scaled +  ww_flam_sample_scaled +  rwc_scaled + excess_water_scaled + site +  year_month + (1 | individual), data = mem_data_all)
...
```{r}
m7 <- lmer(fd ~ mpa_scaled + rwc_scaled+ spp + site + year_month + (1 | individual), data = mem_data_all)

performance::multicollinearity(m7)
```

```{r}
m10 <- lmer(fd ~ rwc_scaled + spp + site + year_month + (1 | individual), data = mem_data_all)
...
```{r}
m12.5 <- lmer(fd ~ dw_flam_sample_scaled +  mpa_scaled + rwc_scaled+ spp + site + year_month + (1 | individual), data = mem_data_all)
...
```{r}
m13 <- lmer(fd ~ excess_water_scaled + rwc_scaled+ spp + site + year_month + (1 | individual), data = mem_data_all)
...
tab_model(mpa_mod_fig.mpa, m11, m7, m10, m3, m6, m6.5, m12, m15, m10, m13, m14, max_model_noww,
  ...
  string.p = "P-Value", 
  p.style = "stars",
  file = here('figures', 'MEM_figures','site_yearmonth_fd_rwc.html')) # saving _fd_rwc.html into figures folder
```
######fd top models: 

The following models are the best models: 
```{r}
tab_model(max_model_noww, m15, m6, m13,
          show.reflvl = TRUE, 
          digits = 3, 
          show.aic = TRUE, 
          show.ci = FALSE,
  ...
  string.p = "P-Value", 
  p.style = "stars",
  file = here('figures', 'MEM_figures','site_yearmonth_topAIC_fd_rwc.html')) # saving _fd_rwc.html into figures folder
```

#fd best model in table: 
```{r}
tab_model(sw_m11, sw_m12.5, max_model_noww,
          show.reflvl = TRUE, 
          digits = 3, 
          show.aic = TRUE, 
          show.ci = FALSE,
  ...
  string.p = "P-Value", 
  p.style = "stars",
  file = here('figures', 'MEM_figures','fd_topAIC_fd_rwc.html')) # saving
```
