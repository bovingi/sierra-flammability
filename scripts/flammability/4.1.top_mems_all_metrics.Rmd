...
tidy_fh <- broom.mixed::tidy(m7_fh)%>% 
  mutate(y_var = "fh")

fh_mod <- lmer(fh ~ mpa_scaled + lfm_scaled+ spp + site + year_month + (1 | individual), data = mem_data_all)
...
performance::multicollinearity(m2)
plot(performance::check_collinearity(m2))
...
performance::multicollinearity(m7)
plot(performance::check_collinearity(m7))

qqnorm(resid(m7))
r <- resid(m7)
plot(fitted(m7), r)

tidy_tti <- broom.mixed::tidy(m7)%>% 
  mutate(y_var = "tti")

tti_mod <- lmer(tti ~ mpa_scaled + lfm_scaled + spp + site + year_month + (1 | individual), data = mem_data_all)
