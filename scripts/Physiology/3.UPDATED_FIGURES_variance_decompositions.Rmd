...
flam_vardecomp_gymno <- ggplot(df_final, aes(x = "Variance Partitioning", y = Variance, fill = Term)) +
  ...
  guides(fill = guide_legend(title = NULL)) + # Remove title from legend
  scale_fill_manual(values =  c(age_new_num = "#fac228", 
             age_new = "#fac228", 
             age = "#fac228",
             spp = "#9f2a63", 
             year = "#d44842", 
             ...
             plant = "#65156e", 
             Residuals = "grey")) + # Manually assign colors from the vcol vector
  scale_x_discrete(
    breaks = seq(0, 100, by = 20), # Define x-axis breaks
    label = seq(100, 0, by = -20), # Label x-axis from 100 down to 0
    expand = c(0, .3) # Add spacing around x-axis categories
...
field_plot_angio <- lfm.vs.mpa.scatterplot_pvflam(f.group = "Angiosperm", y.lab = "Field data", y.axis = T, y.max = 250, df = field_plot_df) +
...
plots <- cowplot::plot_grid(field_plot_angio, 
                            field_plot_gymno, 
                            pv_vd_sp_angio, 
                            pv_vd_sp_gymno, 
                            flam_vd_sp_angio, 
