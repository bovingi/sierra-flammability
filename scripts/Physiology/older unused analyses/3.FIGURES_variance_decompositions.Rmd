...
varPart_log <- fitExtractVarPartModel(lfm_matrix_log, form, angio_field)
varPart_log

field_vardecomp_angio_log <- plotPercentBars.new(varPart, col = c(vcol))+ theme(legend.position = "none")  
...
varPart_log <- fitExtractVarPartModel(lfm_matrix_log, form, gymno_field)
varPart_log

field_vardecomp_gymno_log <- plotPercentBars.new(varPart_log, col = c(vcol))+ theme(legend.position = "none")
...
angio_field <- angio_field %>% 
  mutate(ilfm = 1/lfm) 

lfm_matrix_ilfm <- angio_field %>% 
  ungroup() %>% 
  mutate(id = row_number()) %>% 
  select(id, ilfm) %>% 
  pivot_wider(names_from = id, 
...
varPart_ilfm  <- fitExtractVarPartModel(lfm_matrix_ilfm , form, angio_field)
varPart_ilfm 

field_vardecomp_angio_ilfm  <- plotPercentBars.new(varPart_ilfm , col = c(vcol))+ theme(legend.position = "none")  +
...
gymno_field <- gymno_field %>% 
  mutate(ilfm = 1/lfm) 

lfm_matrix_ilfm <- gymno_field %>% 
  mutate(ilfm = 1/lfm) %>% 
  ungroup() %>% 
  mutate(id = row_number()) %>% 
  select(id, ilfm) %>% 
...
varPart_ilfm  <- fitExtractVarPartModel(lfm_matrix_ilfm , form, gymno_field)
varPart_ilfm 

field_vardecomp_gymno_ilfm  <- plotPercentBars.new(varPart_ilfm , col = c(vcol))+ 
...
## Field Function
```{r}
lfm.vs.mpa.scatterplot <- function(f.group.lab = " ", f.group, y.lab = " ", df, df2, df3, df_pred, y.axis = F, x.axis = F, y.max, present.title = F) {
  
# For color palette
if(f.group == "Angiosperm") {color_pal <- c('#5792CC', '#C75146', '#6E406E','#5792CC', '#C75146', '#6E406E')}
...
plot <- ggplot()  +
  ...
  geom_smooth(data = df_pred1, aes(y = pred_lfm, 
                                  x = -1*water_potential, 
                                  color = spp, 
                                  method = "loess", 
                                  se = F), 
...
####Pred df for plotting lines: 

#$Make predict dataset: 

mod_field <- lm(ilfm ~ water_potential + spp, data = field_data_all)

pred_field_data <- field_data_all %>% 
  group_by(F.Group) %>% 
  do(modelr::add_predictions(., model = mod_field)) %>% 
   do(modelr::add_residuals(., model = mod_field)) %>% 
  mutate(pred_lfm = 1/pred) %>% 
  select(water_potential, F.Group, spp, pred_lfm)
...
field_plot_gymno <- lfm.vs.mpa.scatterplot(f.group.lab = "Gymnosperm", 
                                            ...
                                            df_pred = pred_field_data,
                                            present.title = T) +
  annotate(geom = "text", x = 0, y = 290, label = 'b', fontface = 'bold', size = 12)  + annotation_custom(ggplotGrob(field_vardecomp_gymno),
                                     xmin = 7.1, xmax = 10,
                                     ymin = 95, ymax = 295)
field_plot_gymno
```
...
# For color palette
if(f.group == "Angiosperm") {color_pal <- c('#5792CC', '#C75146', '#6E406E','#5792CC', '#C75146', '#6E406E')}
if(f.group == "Gymnosperm") {color_pal <- c('#42401D', '#5DA493', '#D8AB43','#42401D', '#5DA493', '#D8AB43')}
...
```{r}
legend <- cowplot::plot_grid(blank_plot, legend_dots, legend_tlp, cow_legend, blank_plot,
                             ncol = 1, 
                             rel_heights = c(.7, 1.7, .5, 1, .7))
legend
```
...
plots <- cowplot::plot_grid(field_vd_sp_angio, field_plot_gymno, pv_vd_sp_angio, pv_vd_sp_gymno, flam_vd_sp_angio, flam_vd_sp_gymno, ncol = 2, nrow = 3, rel_heights = c(1.15,1,1.08), rel_widths = c(1.08, 1))
#plots
