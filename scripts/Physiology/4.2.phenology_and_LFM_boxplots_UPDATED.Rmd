...
ggplot() +
  ...
  scale_y_continuous(limits = c(0, 400), sec.axis = sec_axis(trans = ~.*coef, name = 'Water Potential (-MPa)')) +
...
### Function
```{r}
pheno.dotplot.vis <- function(species, phenophases, max.y = 350, colors, no.phenophases = two, pheno_df = pheno_fig2, letter){
...
p1 <- ggplot() +
  ...
  scale_y_continuous(limits = c(0, max.y), sec.axis = sec_axis(trans = ~.*coef, name = 'Water Potential (-MPa)')) +
  theme(axis.ticks.x = element_blank(),
    axis.text.x = element_text(
      angle = 30,
      hjust = 1,
