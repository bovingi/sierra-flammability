...
df_2021 <- readr::read_csv(here("raw-data", "summer2021.mpa.lfm.csv"))
```
#------------------------------------
# Data Wrangling
```{r}
...
write_csv(df_lfm_mpa, here("processed-data", "seki_lfm_mpa_09062022.csv"))
...
df_2021_late_old <- df_2021_late %>% 
  filter(age == 'old') # only old growth
...
New vs. old
```{r}
lfm.vs.mid.late.2021 <- ggplot(data = df_2021_late, aes(x = midday, y = lfm.outliers.out, color = age)) +
  geom_point(size = 0.5) +
  geom_smooth(method = 'lm', se = F, color = 'black', size = 0.3) +
  facet_wrap(~spp) +
  labs(title = "2021 LFM vs. Midday Water Potential (Late Season)", 
