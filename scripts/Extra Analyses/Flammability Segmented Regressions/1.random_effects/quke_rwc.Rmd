---
title: "quke segmented regressions: rwc"
author: "Indra Boving & Joe Celebrezze"
date: "4/15/2021"
output: html_document
---

NOTE: NO THRESHOLDS DETECTED
'Error in MEEM(object, conLin, control$niterEM): Singularity in backsolve at level 0, block 1' kept popping up 
...
rwc_quke_thresholds_seg <- tibble::data_frame(rwc_psi_tti, rwc_st_err_tti, rwc_p_value_tti,
...
rwc_quke_thresholds_rand_seg <- tibble::data_frame(rwc_psi_tti, rwc_st_err_tti,
...
o_mod<-lme(tti~rwc + year_month + site, random=~1|id, data=quke_subset_seg_tti)
...
#put into the df:
rwc_quke_thresholds_rand_seg[1] <- summary(os_mod_b)$psi[2]
rwc_quke_thresholds_rand_seg[2] <- summary(os_mod_b)$psi[3]

# Segmented regression no random changepoints: 

u_mod <- lm(tti~rwc + year_month + site, data = quke_subset_seg_tti)
...
#put into the df:
rwc_quke_thresholds_seg[1] <- summary(u_mod_b)$psi[2]
rwc_quke_thresholds_seg[2] <- summary(u_mod_b)$psi[3]
rwc_quke_thresholds_seg[3] <- u_mod_davies$p.value


rsegplot.rwc(os_mod_b, quke_subset_seg_tti, max.x = quke_max_rwc, y.variable = 'tti', y.lab = "Time to Ignition (s)")
```
...
o_mod<-lme(fh~rwc + year_month + site, random=~1|id, data=quke_subset_seg_fh)
os_mod<-segmented.default(o_mod, ~rwc, npsi=list(rwc=1))
#summarizing results (note the '_coef' argument)
summary.segmented(os_mod)

#put into the df:
rwc_quke_thresholds_rand_seg[3] <- summary(os_mod)$psi[2]
rwc_quke_thresholds_rand_seg[4] <- summary(os_mod)$psi[3]

# Segmented regression no random changepoints: 

u_mod <- lm(fh~rwc + year_month + site, data = quke_subset_seg_fh)
u_mod_b <- segmented(u_mod, ~rwc)
summary.segmented(u_mod_b)
...
#put into the df:
rwc_quke_thresholds_seg[4] <- summary(u_mod_b)$psi[2]
rwc_quke_thresholds_seg[5] <- summary(u_mod_b)$psi[3]
rwc_quke_thresholds_seg[6] <- u_mod_davies$p.value

rsegplot.rwc(os_mod, quke_subset_seg_fh, max.x = quke_max_rwc, y.variable = 'fh', y.lab = "Flame Height (cm)")
```
...
o_mod<-lme(fd~rwc + year_month + site, random=~1|id, data=quke_subset_seg_fd)
os_mod <-segmented.default(o_mod, ~rwc, npsi=list(rwc=1))
#summarizing results (note the '_coef' argument)
summary.segmented(os_mod)

#put into the df:
rwc_quke_thresholds_rand_seg[5] <- summary(os_mod)$psi[2]
rwc_quke_thresholds_rand_seg[6] <- summary(os_mod)$psi[3]

# Segmented regression no random changepoints: 

u_mod <- lm(fd~rwc + year_month + site, data = quke_subset_seg_fd)
u_mod_b <- segmented(u_mod, ~rwc)
summary.segmented(u_mod_b)
...
#put into the df:
rwc_quke_thresholds_seg[7] <- summary(u_mod_b)$psi[2]
rwc_quke_thresholds_seg[8] <- summary(u_mod_b)$psi[3]
rwc_quke_thresholds_seg[9] <- u_mod_davies$p.value

rsegplot.rwc(os_mod, quke_subset_seg_fd, max.x = quke_max_rwc, y.variable = 'fd', y.lab = "Flame Duration (s)")
```
...
o_mod<-lme(gd~rwc + year_month + site, random=~1|id, data=quke_subset_seg_gd)
os_mod <-segmented.default(o_mod, ~rwc, npsi=list(rwc=1))
#summarizing results (note the '_coef' argument)
summary.segmented(os_mod)

#put into the df:
rwc_quke_thresholds_rand_seg[7] <- summary(os_mod)$psi[2]
rwc_quke_thresholds_rand_seg[8] <- summary(os_mod)$psi[3]

# Segmented regression no random changepoints: 

u_mod <- lm(gd~rwc + year_month + site, data = quke_subset_seg_gd)
u_mod_b <- segmented(u_mod, ~rwc)
summary.segmented(u_mod_b)
...
#put into the df:
rwc_quke_thresholds_seg[10] <- summary(u_mod_b)$psi[2]
rwc_quke_thresholds_seg[11] <- summary(u_mod_b)$psi[3]
rwc_quke_thresholds_seg[12] <- u_mod_davies$p.value

rsegplot.rwc(os_mod, quke_subset_seg_gd, max.x = quke_max_rwc, y.variable = 'gd', y.lab = "Glow Duration (s)")
```
...
o_mod<-lme(prop_ignite~rwc + year_month + site, random=~1|id, quke_subset_seg_prop_ignite )
os_mod<-segmented.default(o_mod, ~rwc, npsi=list(rwc=1))
#summarizing results (note the '_coef' argument)
summary.segmented(os_mod)

#put into the df:
rwc_quke_thresholds_rand_seg[9] <- summary(os_mod)$psi[2]
rwc_quke_thresholds_rand_seg[10] <- summary(os_mod)$psi[3]

# Segmented regression no random changepoints: 

u_mod <- lm(prop_ignite~rwc + year_month + site, data = quke_subset_seg_prop_ignite)
u_mod_b <- segmented(u_mod, ~rwc)
summary.segmented(u_mod_b)
...
#put into the df:
rwc_quke_thresholds_seg[13] <- summary(u_mod_b)$psi[2]
rwc_quke_thresholds_seg[14] <- summary(u_mod_b)$psi[3]
rwc_quke_thresholds_seg[15] <- u_mod_davies$p.value

rsegplot.rwc(os_mod, quke_subset_seg_prop_ignite, max.x = quke_max_rwc, y.variable = 'prop_ignite', y.lab = "Proportion Ignted (5% Bins of rwc)")
```
...
o_mod<-lme(lower_temp_max~rwc + year_month + site, random=~1|id, data=quke_subset_seg_temp_change)
os_mod<-segmented.default(o_mod, ~rwc, npsi=list(rwc=1))
#summarizing results (note the '_coef' argument)
summary.segmented(os_mod)
os_mod
...
#put into the df:
rwc_quke_thresholds_rand_seg[11] <- summary(os_mod)$psi[2]
rwc_quke_thresholds_rand_seg[12] <- summary(os_mod)$psi[3]

# Segmented regression no random changepoints: 

u_mod <- lm(temp_change~rwc + year_month + site, data = quke_subset_seg_temp_change)
u_mod_b <- segmented(u_mod, ~rwc)
summary.segmented(u_mod_b)
...
#put into the df:
rwc_quke_thresholds_seg[16] <- summary(u_mod_b)$psi[2]
rwc_quke_thresholds_seg[17] <- summary(u_mod_b)$psi[3]
rwc_quke_thresholds_seg[18] <- u_mod_davies$p.value

rsegplot.rwc(os_mod, quke_subset_seg_temp_change, max.x = quke_max_rwc, y.variable = 'temp_change', y.lab = "Temp Change (C)")
```

# Final Table 
```{r}
#No random effects: 
quke_rwc_summary_df <- rwc_quke_thresholds_seg %>% 
...
#with random effects: 
quke_rwc_rand_summary_df <- rwc_quke_thresholds_rand_seg %>% 
  pivot_longer(everything(), 
               names_to = "flam_metric", 
              values_to = "value") %>% 
  group_by(flam_metric) %>% 
...
quke_rwc_rand_summary_df

quke_rwc <- bind_rows(quke_rwc_rand_summary_df, quke_rwc_summary_df) %>% 
  mutate(spp = "quke", 
         water_variable = "rwc") 

#ggsave(plot=gridExtra::tableGrob(quke_rwc), filename=here("figures", "segmented_tables", "rand_segmented_rwc_quke.png"))
```
