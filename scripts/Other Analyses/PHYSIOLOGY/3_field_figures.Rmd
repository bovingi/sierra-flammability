...
field_data_all_3 %>% 
  ...
  labs(title = "September 2021 CZO LFM data, variation across plants")
```
...
p1 <- field_data_all_3 %>% 
 filter(#
   age == "new", strategy == "Isohydric") %>% #filtering to 'old' so that we dont have duplicate MPa datapoints
  #filter(site == 1) %>% 
...
p2 <- field_data_all_3 %>% 
 filter(#age == "new", strategy == "Anisohydric"
...
field_data_all_3 %>%
  ...
  geom_point(size = 1.4, alpha = .65, aes(color = Species)) +
  geom_vline(
...
field_data_all_3 %>% 
  filter(month %in% c(9)) %>% 
  group_by(spp, year) %>% 
  summarise(mean_lfm = mean(lfm), 
            mean_mpa = mean(midday)) %>% 
  arrange(year, mean_lfm, mean_mpa)
```
...
#MPa over time: 
```{r}
mpa_plot <- field_data_all_3 %>%
  mutate(spp = fct_relevel(spp, "ABCO", "PIJE", "CADE", "CECO", "ARPA", "QUKE"))  %>%
  filter(age != "both",
         lfm < 350) %>%
  filter(year == 2021) %>% 
  pivot_longer(cols = c(midday, predawn), 
               names_to = "time_mpa", 
               values_to = "field_mpa", 
               names_repair = "minimal") 
#%>% 
  # select(lfm, spp, mean_tlp_spp, sd_spp, age, F.Group, date, field_mpa, timing)

mpa_plot %>% 
   mutate(spp = fct_relevel(spp, "ABCO", "PIJE", "CADE", "CECO", "ARPA", "QUKE"))  %>%
  ggplot(aes(y = field_mpa,
             x = cut(date, breaks = "week"),
             fill = time_mpa)) +
  geom_boxplot() +
  facet_wrap(~ spp, 
              labeller = labeller(spp = spp.labs), 
              scales = "free") +
  geom_hline(aes(yintercept  = mean_tlp_spp), field_data_all_3,
             linetype = "dotted")  +
  theme(axis.ticks.x = element_blank(),
    #axis.title.x = element_blank(),
    axis.text.x = element_text(
...
ggplot() +
  ...
  geom_hline(aes(yintercept  = mpatlp), field_data_all_3,
             linetype = "dotted") +
  # geom_line(aes(y = lfm, 
  #               #x = cut(date, breaks = "week"),
  #                x = week,
...
df1$month_char <- factor(df1$month_char, levels = c("E. April", "L. April", "E. May", "L. May", 
                                          "E. June", "L. June","E. July",
                                          "L. July", "E. Aug.", "L. Aug.", "E. Sept.",
                                          "L. Sept.", "E. Oct.", "L. Oct."))
