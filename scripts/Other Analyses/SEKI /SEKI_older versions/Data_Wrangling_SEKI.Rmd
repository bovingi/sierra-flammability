---
title: 'Data Wrangling: seki Flam. Curves (SB Area)'
author: "Indra Boving"
date: "2/19/2021"
...
```{r}
raw.data.seki <- read_csv(here("raw-data/SEKI_flammability.csv")) %>% 
  filter(lfm > 0) #remove columns with missing dry weight/neg. lfm
str(raw.data.seki)
```
...
data.with.na.seki[c("ttfg", "tti", "fd", "gd", "pfg", "fh", "mpa",  "prop.new", "water.wt", "fresh.wt", "dry.wt", "start.temp")][(data.with.na.seki[c("ttfg", "tti", "fd", "gd", "pfg", "fh", "mpa", "prop.new", "water.wt", "fresh.wt", "dry.wt", "start.temp")] == 0)] <- NA
...
data.with.na.seki[c("ttfg", "tti", "fd", "gd", "pfg", "gti")][(data.with.na.seki[c("ttfg", "tti", "fd", "gd", "pfg", "gti")] > 1000)] <- NA
data.with.na.seki[c("water.wt")][(data.with.na.seki[c("water.wt")] > 8 )] <- NA
```
...
outlierKD2(data.with.na.seki, lfm.outliers.out) #check for outliers and remove (happens in outliers.out column, lfm column still contains outliers)
```
...
data <- data.with.na.seki %>% 
 ...
 # mutate(ignition2 = as.numeric(ignition2)) 
  mutate(ignition2 = dplyr::recode(as.double(ignition2), `2` = 1))
...
write.csv(rwc.data, here("processed-data/seki.flam.curve.physiological.data.csv"))
...
First, to visualize NA values in flam metrics:
- Ignited samples only, both models
...
Overall, not too many missing values, so maybe we dont need to go too crazy with this. Also, flame height can be considered MCAR here due to flame heights not going out of screen in Epi trials. 
...
ignite.only.epi <- ignite.only.epi %>%
  ...
  impute_median_at(c("ttfg", "gd", "gti", "fd", "pfg","temp.max", "start.temp", "ros", "ignition.temp", "temp.change", "prop.ignite", "fh")) 

colSums(is.na(ignite.only.epi)) #should not be any NAs except in LFM.outliers.out
...
ignite.only.hp <- ignite.only.hp %>%
  group_by(year.month) %>% 
  group_by(spp) %>% #this makes us take the median of based on the species, the model, and the lfm bins (of 10 lfm) the missing value is in. Preserves species differences, and lfm impact. 
  impute_median_at(c("ttfg", "gd", "gti", "fd", "pfg","temp.max", "start.temp", "ros", "ignition.temp", "temp.change", "prop.ignite", "ros", "tti")) %>% 
...
range(ignite.only.epi$gd) 
range(ignite.only.epi$pfg) 
range(ignite.only.epi$ttfg)
range(ignite.only.epi$dw.flam.sample)
range(ignite.only.epi$ww.flam.sample)

ignite.only.epi <- ignite.only.epi %>%
  mutate(flam.index = 3.4*((75+tti)/(12.5+tti))*exp(fh/(fh+27))) #using max tti and max fh, becomes a scale of least (1) to most (20) flammable 

range(ignite.only.epi$flam.index)
```


```{r}
ttiplot <- ignite.only.epi %>% 
...
ignite.only.hp <- ignite.only.hp %>%
  mutate(flam.index = 1.2*((252+tti)/(12.5+tti))*exp(fh/(fh+58))) #using max tti and max fh, becomes a scale of least (1) to most (20) flammable

range(ignite.only.hp$flam.index)
```

Combine back into master dataset: 
```{r}
ignite.only.epi
ignite.only.hp
...
Here we are normalizing by the dry weight of the flammability sample, which was calculated by multiplying the weight of the burned sample by the ratio of dry weight to fresh weight in the samples used for lfm measurements. 
...
range(ignite.only.epi$dry.norm.gti)
range(ignite.only.epi$dry.norm.tti)
range(ignite.only.epi$dry.norm.fh)
range(ignite.only.epi$dry.norm.gd) 
range(ignite.only.epi$dry.norm.pfg) 
range(ignite.only.epi$dry.norm.ttfg)
range(ignite.only.epi$dw.flam.sample)
...
```{r}
epi.ignite.only.PCA <- ignite.only.epi %>%
  mutate(mpa = mpa * -1) 
  ...
  # relocate("hydration")
colSums(is.na(epi.ignite.only.PCA))

write.csv(x = epi.ignite.only.PCA, (here("processed-data", "epi.ignite.only.seki.csv")))
```
...
write.csv(x = hp.ignite.only.pca, (here("processed-data", "hp.ignite.only.seki.csv")))
```
